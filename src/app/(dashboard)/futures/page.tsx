                  ...
                  return (
                    ...
                    >
                      <td className="px-3 py-2.5">
                        <span className={cn("font-bold text-xs", regionColors[f.region] ?? "text-foreground")}>{f.symbol}</span>
                      </td>
          ...
          <div>
            ...
            {portfolioMarginPct > 80 && (
              <div className="mt-1 flex items-center gap-1 text-xs text-red-400">
                <AlertTriangle className="h-3 w-3" /> Margin usage exceeds 80% — margin call risk!
              </div>
            )}
          </div>
      ...
      <Tabs defaultValue="equity" className="w-full mt-8">
        <TabsList className="flex w-full flex-wrap gap-0 rounded-lg border border-border bg-muted/30 p-1 h-auto">
          {[
            ...
            { value: "rates",    label: "Interest Rates",    icon: <DollarSign   className="h-3.5 w-3.5" /> },
            { value: "currency", label: "Currency & Crypto", icon: <Globe        className="h-3.5 w-3.5" /> },
            { value: "vol",      label: "Volatility",        icon: <Zap          className="h-3.5 w-3.5" /> },
            { value: "margin",   label: "Order & Margin",    icon: <Calculator   className="h-3.5 w-3.5" /> },
          ].map((t) => (
            <TabsTrigger
              key={t.value}
              value={t.value}
