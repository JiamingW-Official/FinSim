...
function REITUniverseTab() {
 ...
 { label: "Avg P/FFO", value: fmt(REITS.reduce((a, r) => a + r.pFfo, 0) / REITS.length, 1) + "x", icon: <Calculator className="w-3.5 h-3.5 text-muted-foreground/50" /> },
...
function ValuationTab() {
 ...
 className="w-full bg-muted/40 border border-border rounded-md px-3 py-1.5 text-sm font-mono"
 />
 </div>
 </div>
 ...
 <Card key={s.label} className={`border-border ${s.highlight ? "bg-muted/10 border-primary/30" : "bg-muted/30"}`}>
 <CardContent className="p-3">
 <p className="text-xs text-muted-foreground">{s.label}</p>
...
function DividendsTab() {
 ...
 { label: "3Y Div CAGR", value: `${cagr.toFixed(1)}%`, icon: <TrendingUp className="w-3.5 h-3.5 text-muted-foreground/50" /> },
...
function PortfolioTab() {
 ...
 <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
 {[
 { label: "Holdings", value: `${portfolio.length} REITs`, icon: <Building2 className="w-3.5 h-3.5 text-muted-foreground/50" /> },
...
export default function REITsPage() {
 ...
 {/* Hero */}
 <h1 className="text-3xl font-bold tracking-tight text-foreground mb-1">REITs</h1>
 <p className="text-[10px] uppercase tracking-widest text-muted-foreground/40 mb-6">REAL ESTATE · NAV · FFO · DISTRIBUTION</p>
