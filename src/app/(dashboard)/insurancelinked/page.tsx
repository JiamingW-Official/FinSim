...
function MetricCard({
 ...
 const colors = {
 blue: "text-foreground bg-muted/10",
 green: "text-green-400 bg-green-500/10",
 amber: "text-amber-400 bg-amber-500/10",
 purple: "text-foreground bg-muted/10",
 };
 return (
...
function PerilBadge({ peril }: { peril: CatBond["peril"] }) {
 ...
 Hurricane: { icon: Wind, cls: "bg-muted/10 text-foreground border-border" },
 ...
 "Multi-Peril": { icon: Globe, cls: "bg-muted/10 text-foreground border-border" },
 Flood: { icon: Droplets, cls: "bg-teal-500/10 text-emerald-400 border-teal-500/20" },
 };
 const { icon: Icon, cls } = map[peril];
 return (
 <span className={cn("inline-flex items-center gap-1 text-xs text-muted-foreground px-2 py-0.5 rounded-full border font-medium", cls)}>
 {peril}
 </span>
 );
}
...
function TriggerBadge({ trigger }: { trigger: CatBond["trigger"] }) {
 ...
 "Industry Index": "bg-muted/10 text-foreground border-border",
 Parametric: "bg-orange-500/10 text-orange-400 border-orange-500/20",
 "Modeled Loss": "bg-muted/10 text-foreground border-border",
 };
 return (
 <span className={cn("text-xs text-muted-foreground px-2 py-0.5 rounded-full border font-medium", colors[trigger])}>
 {trigger}
 </span>
 );
}
...
export default function InsuranceLinkedPage() {
 ...
 <Badge variant="outline" className="text-xs border-border text-foreground">Alternative Asset</Badge>
 ...
 {/* ── Cat Bonds Tab ── */}
 <TabsContent value="catbonds" className="space-y-4 data-[state=inactive]:hidden">
 {/* Issuance Chart */}
 <Card className="bg-card border-border">
 ...
 <Badge variant="outline" className="text-xs text-muted-foreground border-border">{b.rating}</Badge>
 </td>
 </tr>
 ))}
 </tbody>
 ...
 {[
 { icon: Info, color: "text-foreground", title: "Trigger Types", body: "Indemnity triggers pay on actual insurer losses, reducing basis risk. Parametric triggers pay on measured physical parameters (wind speed, magnitude). Industry index and modeled loss triggers sit between these extremes." },
 ...
 { icon: Globe, color: "text-foreground", title: "Diversification Value", body: "ILS has near-zero correlation to equity and credit markets. Natural catastrophe frequency is independent of economic cycles. This makes ILS a powerful portfolio diversifier and return enhancer in multi-asset portfolios." },
 ...
 {/* ── ILS Funds Tab ── */}
 <TabsContent value="ilsfunds" className="space-y-4 data-[state=inactive]:hidden">
 {/* Risk-Return Scatter */}
 <Card className="bg-card border-border">
 <CardHeader className="pb-2">
 ...
 <td className="px-4 py-2.5 text-foreground">{f.sharpe.toFixed(2)}</td>
 ...
 <div>
 <p className="text-sm font-medium text-foreground mb-1">Investor Access</p>
 ...
 {/* ── Risk Modeling Tab ── */}
 <TabsContent value="riskmodel" className="space-y-4 data-[state=inactive]:hidden">
 {/* Loss histogram */}
 <Card className="bg-card border-border">
 <CardHeader className="pb-2">
 <CardTitle className="text-sm font-medium flex items-center gap-2">
 ...
 <TabsContent value="history" className="space-y-4 data-[state=inactive]:hidden">
 {/* Events table */}
 <Card className="bg-card border-border">
 <CardHeader className="pb-2">
 <CardTitle className="text-sm font-medium flex items-center gap-2">
 ...
 <td className="px-4 py-2.5">
 <span className={cn("text-xs text-muted-foreground px-2 py-0.5 rounded-full border font-medium", impact.cls)}>{impact.label}</span>
 </td>
 </tr>
 );
 })}
