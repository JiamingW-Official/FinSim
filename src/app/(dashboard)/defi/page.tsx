...
function TypeBadge({ type }: { type: string }) {
 ...
 lending: "bg-muted/10 text-foreground border-border",
 lp: "bg-muted/10 text-foreground border-border",
...
function YieldDashboard() {
 ...
 <td className={cn("p-3 text-right font-semibold", totalAPY >= 20 ? "text-rose-400" : totalAPY >= 10 ? "text-amber-400" : "text-emerald-400")}>
 {fmtPct(totalAPY)}
 </td>
...
function LendingAnalyzer() {
 ...
 <div className={cn("text-2xl font-semibold", healthFactor >= 2 ? "text-emerald-400" : healthFactor >= 1.3 ? "text-amber-400" : "text-rose-400")}>
 ...
 {[
 { step: "1", action: "Borrow $10M USDC (0% collateral)", color: "text-foreground" },
 ...
 { step: "5", action: "Keep arbitrage profit (all in 1 transaction)", color: "text-foreground" },
 ].map((s) => (
...
function ProtocolRankings() {
 ...
 <div className="flex-1 bg-foreground/5 rounded h-5 relative overflow-hidden">
 <div
 className="h-full bg-indigo-500/40 rounded transition-colors"
