...
function FilterChip({ f, onRemove, onToggle }: { f: FilterCriterion; onRemove: () => void; onToggle: () => void }) {
 ...
 <div
 className={cn(
 "flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs text-muted-foreground cursor-pointer select-none transition-colors",
 f.enabled
...
export default function ScreenerPage() {
 ...
 AI <span className="font-light text-muted-foreground/40">Screener</span>
 </h1>
 ...
 className="rounded-full border border-border text-xs font-medium px-4 py-1.5 text-muted-foreground hover:text-foreground transition-colors"
 >
 Save
 </button>
 <button
 ...
 className="rounded-full bg-foreground text-background text-xs font-semibold px-4 py-1.5 hover:bg-foreground/90 transition-colors"
 >
 Run Screen
 </button>
 </>
 ...
 <p className="text-[11px] font-mono text-muted-foreground/30 mt-1.5">50 synthetic stocks · AI-scored · Seeded universe</p>
 </div>

 <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-1 flex-col overflow-hidden">
 <TabsList className="mx-5 mt-3 mb-0 shrink-0 w-fit h-9 bg-transparent gap-0 p-0 rounded-none border-b border-transparent">
 <TabsTrigger value="builder" className={cn("rounded-none border-b-2 px-4 pb-2 pt-1 text-xs font-medium transition-colors bg-transparent shadow-none", activeTab === "builder" ? "border-foreground text-foreground font-semibold" : "border-transparent text-muted-foreground/50 hover:text-muted-foreground")}>Screen Builder</TabsTrigger>
 <TabsTrigger value="results" className={cn("rounded-none border-b-2 px-4 pb-2 pt-1 text-xs font-medium transition-colors bg-transparent shadow-none", activeTab === "results" ? "border-foreground text-foreground font-semibold" : "border-transparent text-muted-foreground/50 hover:text-muted-foreground")}>
 Results {hasRun && <span className="ml-1.5 rounded font-mono tabular-nums px-1.5 py-0.5 text-[10px] bg-foreground/10 text-foreground">{filteredStocks.length}</span>}
 </TabsTrigger>
 <TabsTrigger value="ai" className={cn("rounded-none border-b-2 px-4 pb-2 pt-1 text-xs font-medium transition-colors bg-transparent shadow-none", activeTab === "ai" ? "border-foreground text-foreground font-semibold" : "border-transparent text-muted-foreground/50 hover:text-muted-foreground")}>AI Recommendations</TabsTrigger>
 <TabsTrigger value="saved" className={cn("rounded-none border-b-2 px-4 pb-2 pt-1 text-xs font-medium transition-colors bg-transparent shadow-none", activeTab === "saved" ? "border-foreground text-foreground font-semibold" : "border-transparent text-muted-foreground/50 hover:text-muted-foreground")}>Saved Screens</TabsTrigger>
 </TabsList>
 ...
 <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground/30">Active Filters</p>
 <span className="text-[10px] font-mono text-muted-foreground/40 tabular-nums">{criteria.filter((c) => c.enabled).length} enabled · {criteria.length} total</span>
 </div>
 {criteria.length === 0 ? (
 ...
 <div className="rounded-xl border border-border bg-card p-4">
 <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground/30 mb-3">Market Cap</p>
 <div className="flex flex-wrap gap-2">
 ...
 className={cn(
 "rounded text-[10px] font-mono uppercase tracking-wide px-2.5 py-1 transition-colors",
 marketCapFilter === cat
 ? "bg-foreground text-background font-semibold"
 : "border border-border/60 text-muted-foreground/50 hover:border-foreground/20 hover:text-foreground",
 )}
 >
 ...
 <div className="rounded-xl border border-border bg-card p-4">
 <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground/30 mb-3">Sector</p>
 <div className="flex flex-wrap gap-2">
 <button
 type="button"
 ...
 <div className="rounded-xl border border-border bg-card p-4">
 <div className="mb-3 flex items-center justify-between">
 <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground/30">Add Criterion</p>
 <button
 type="button"
 onClick={() => setAddFilterOpen((v) => !v)}
 className="rounded-full bg-foreground text-background text-[11px] font-semibold px-3 py-1 hover:bg-foreground/90 transition-colors flex items-center gap-1"
 >
 ...
 className={cn(
 "rounded-md border px-3 py-1 text-xs text-muted-foreground font-medium transition-colors",
 ...
 className="cursor-pointer whitespace-nowrap px-4 py-3 text-left text-[10px] font-mono uppercase tracking-[0.15em] text-muted-foreground/40 font-medium hover:text-muted-foreground select-none"
 >
 ...
 className={cn(
 "cursor-pointer transition-colors duration-100 group hover:bg-foreground/[0.02]",
 selectedStock?.ticker === s.ticker && "bg-foreground/[0.03]",
 )}
 >
 <td className="px-4 py-3 font-mono tabular-nums text-sm text-muted-foreground/40">{s.rank}</td>
 <td className="px-4 py-3"><span className="font-mono font-semibold text-sm text-foreground">{s.ticker}</span></td>
 <td className="px-4 py-3 max-w-[120px] truncate text-sm text-muted-foreground">{s.company}</td>
 <td className="px-4 py-3 text-sm text-muted-foreground/60">{s.sector.split(" ")[0]}</td>
 <td className="px-4 py-3">
 <span className={cn(
 "inline-flex items-center rounded px-2 py-0.5 text-[11px] font-mono font-semibold tabular-nums",
 s.score >= 70 ? "bg-emerald-500/10 text-emerald-400" : s.score >= 50 ? "bg-amber-500/10 text-amber-400" : "bg-red-500/10 text-red-400"
 )}>{s.score}</span>
 </td>
 <td className="px-4 py-3">
 <span className={cn("font-mono tabular-nums text-sm", s.peRatio < 15 ? "text-emerald-400" : s.peRatio > 35 ? "text-red-400" : "text-muted-foreground")}>
 {s.peRatio}x
 </span>
 </td>
 <td className="px-4 py-3">
 <span className={cn("font-mono tabular-nums text-sm", s.forwardPE < 18 ? "text-emerald-400" : s.forwardPE > 30 ? "text-red-400" : "text-muted-foreground")}>
 {s.forwardPE}x
 </span>
 </td>
 <td className="px-4 py-3">
 <span className={cn("font-mono tabular-nums text-sm", s.revenueGrowth > 15 ? "text-emerald-400" : s.revenueGrowth < 0 ? "text-red-400" : "text-amber-400")}>
 {s.revenueGrowth > 0 ? "+" : ""}{s.revenueGrowth}%
 </span>
 </td>
 <td className="px-4 py-3">
 <span className={cn("font-mono tabular-nums text-sm", s.netMargin > 10 ? "text-emerald-400" : s.netMargin < 0 ? "text-red-400" : "text-amber-400")}>
 {s.netMargin}%
 </span>
 </td>
 <td className="px-4 py-3">
 <span className={cn("font-mono tabular-nums text-sm", rsiColor(s.rsi))}>{s.rsi}</span>
 </td>
 <td className="px-4 py-3 font-mono tabular-nums text-sm text-foreground">${s.price.toFixed(2)}</td>
 <td className="px-4 py-3">
 <span className={cn("font-mono tabular-nums text-sm", returnColor(s.return1M))}>
 {s.return1M > 0 ? "+" : ""}{s.return1M}%
 </span>
 </td>
 <td className="px-4 py-3 font-mono tabular-nums text-sm text-muted-foreground">{formatMarketCap(s.marketCapB)}</td>
 <td className="px-4 py-3">
 <span className={cn("font-mono tabular-nums text-sm", s.dividendYield > 0 ? "text-emerald-400" : "text-muted-foreground/40")}>
 ...
 <p className={cn("text-xs text-muted-foreground", returnColor(selectedStock.return1M))}>
 ...
 <p className={cn("text-xs text-muted-foreground", returnColor(pick.return1M))}>
 ...
 <p className="text-xs text-muted-foreground leading-relaxed">{pick.rationale}</p>
 </div>
 ...
 <span className={cn("text-xs text-muted-foreground font-medium", scoreColor(pick.score))}>Score {pick.score}/100</span>
 ...
 <tbody>
 {sectorLeaders.map((s) => (
 <tr key={s.ticker} className="border-b border-border hover:bg-muted/10 transition-colors">
 ...
 <span className={cn("text-xs text-muted-foreground font-medium", scoreColor(screen.resultCount * 5))}>
 ...
 <p className="text-xs text-muted-foreground font-medium mb-1">{screen.name}</p>
 ...
 <p className="text-xs text-muted-foreground font-medium">{cs.name}</p>
 ...
 <p className="text-xs text-muted-foreground font-medium">{cs.resultCount} stocks</p>
