...
function MetricCard({ label, value, sub, trend }: {
 ...
 <div className={cn("text-xl font-semibold", trendColor)}>{value}</div>
...
function StructureTab() {
 ...
 <span className="font-semibold text-foreground text-sm">Conduit CMBS</span>
 </div>
 ...
 <div className="bg-muted/60 border border-border rounded-md p-5 space-y-3">
 <div className="flex items-center gap-2">
 <span className="font-semibold text-foreground text-sm">Single-Asset / Single-Borrower (SASB)</span>
 </div>
 <p className="text-xs text-muted-foreground leading-relaxed">
 ...
 { role: "Trustee / Cert. Admin", desc: "Holds mortgage files, distributes principal & interest per waterfall. Maintains transaction registers and tax reporting.", color: "text-foreground" },
...
function LoanAnalysisTab() {
 ...
 <div
 className={cn("h-1.5 rounded-full transition-colors duration-300", loan.stressedDSCR >= 1.25 ? "bg-green-500" : loan.stressedDSCR >= 1.0 ? "bg-amber-500" : "bg-red-500")}
...
function PerformanceTab() {
 ...
 <div
 className={cn("h-2 rounded-full transition-colors", v.lossRate > 3 ? "bg-red-500" : v.lossRate > 1.5 ? "bg-amber-500" : "bg-green-500")}
 ...
 <div className="flex-1 bg-muted rounded-full h-2">
 <div className="h-2 rounded-full transition-colors" style={{ width: `${(p.cpr / 25) * 100}%`, backgroundColor: color }} />
 </div>
...
function TradingTab() {
 ...
 { step: "4. Post-Trade", items: ["Update position risk systems", "Monitor servicer advances", "Watch delinquency triggers", "Report to PM & risk committee"], color: "border-border text-foreground" },
 ].map((phase) => (
