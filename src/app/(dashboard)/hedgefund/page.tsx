      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        {STRATEGIES.map((st) => (
          ...
          >
            ...
            <div className="text-[10px] text-zinc-500">Gross: {st.grossLeverage}</div>
          </motion.button>
        ))}
      </div>
                ...
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      <Badge className={cn("text-[10px] px-1.5 py-0 h-5 gap-1", categoryColor(theme.category))}>
              ...
              <div className="space-y-2 flex-1">
                ...
                {MACRO_REGIMES.map((r, i) => (
                  ...
                  >
                    ...
                    <div className="text-zinc-500 text-[10px]">{r.quadrant}</div>
                  </button>
                ))}
                <div className="mt-3 rounded-lg bg-white/5 p-3">
                  <p className="text-[10px] text-zinc-500 uppercase tracking-wide mb-1.5">Recommended trades in {regime.name}</p>
                  {regime.trades.map((t) => (
              ...
              {TREND_MARKETS.map((m) => (
                <tr key={m.name} className="border-b border-white/5">
                  ...
                  <td className="py-1.5 px-3">
                    <Badge className={cn("text-[10px] px-1.5 py-0 h-5", signalColor(m.signal))}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
              ...
              <tbody>
                {hwmScenarios.map((row) => (
                  <tr key={row.year} className="border-b border-white/5">
                    ...
                    <td className="py-1.5 text-zinc-500 text-[10px]">
        ...
        <Card className="bg-white/3 border-white/8">
          ...
          <CardContent className="pt-0 space-y-4">
            ...
            <p className="text-[10px] text-zinc-600">* Perf fee assumes 10% gross return, 5% hurdle, no HWM adjustment</p>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              {[
                ...
                <div key={label} className="rounded-lg bg-white/5 p-2.5">
                  ...
                  <div className="text-zinc-600 text-[10px]">{note}</div>
                </div>
              ))}
            </div>
          </CardContent>
