...
function GreekBar({ label, value, max, color, desc }: { label: string; value: number; max: number; color: string; desc: string }) {
 ...
 <span className={cn("text-sm font-mono font-semibold", color)}>{value >= 0 ? "+" : ""}{value.toFixed(4)}</span>
 </div>
 <div className="h-2 rounded-full bg-foreground/5 overflow-hidden">
 <div className={cn("h-full rounded-full transition-colors", color.replace("text-", "bg-"))} style={{ width: `${pct}%` }} />
 </div>
...
export default function ConvertibleBondsPage() {
 ...
 { value: "mandatory", label: "Mandatory CBs" },
 { value: "arb", label: "CB Arb" },
 { value: "structure", label: "Capital Structure" },
 ].map((t) => (
 <TabsTrigger
 key={t.value}
 value={t.value}
 ...
 <p><span className="text-foreground font-medium">Purple curve</span> = CB market price. Bounded below by the investment floor and above tracks parity with a diminishing premium.</p>
 ...
 <div className="text-foreground font-medium mb-1">Gamma — Credit-Modified</div>
 ...
 { label: "Gamma", value: selectedDerived.gamma.toFixed(5), color: "text-foreground", sub: "Delta velocity" },
 ...
 { title: "Gamma Profile Risk", color: "text-foreground", desc: "Gamma is highest near conversion price. If stock moves away, dynamic rebalancing costs eat into profits. Must rehedge frequently." },
