...
export default function MicrostructureDepthPage() {
 ...
 { label: "Kyle λ (large-cap)", value: kyleLambdaAssets[0].lambda, color: "text-foreground border-border bg-muted/5" },
 { label: "√-Impact @$1M", value: `${(sigma * Math.sqrt(1 / adv) * kappa * 10000).toFixed(2)}bp`, color: "text-foreground border-border bg-muted/5" },
 ].map((chip) => (
 ...
 <p className="text-xs text-foreground font-mono">S = 2√(-Cov(Δp_t, Δp_t-1))</p>
 ...
 <p className="text-foreground font-medium mb-0.5">Permanent Impact</p>
 ...
 <CardContent>
 <div className="p-3 mb-4 bg-muted/5 border border-border rounded-lg font-mono text-xs text-foreground">
 Impact(bps) = σ · √(Q / ADV) · κ
 <p className="text-muted-foreground mt-1 font-sans">σ={sigma} · κ={kappa} · ADV=$50M · Q=$<span className="text-foreground font-mono">{qm}M</span></p>
 </div>
 <div className="mb-4">
 ...
 <CardContent>
 <div className="mb-3 p-3 bg-muted/5 border border-border rounded-lg text-xs font-mono text-foreground">
 Δp = λ · (Q_buy − Q_sell) + ε
 ...
 asset.liquidity === "High" ? "border-border text-foreground" :
 ...
 <p className="text-base font-medium text-foreground font-mono">{asset.lambda}</p>
