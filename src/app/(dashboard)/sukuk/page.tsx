...
function ShariahTab() {
 ...
 <span className="font-semibold text-foreground">{p.arabic}</span>
 ...
 <tr
 key={row.sector}
 className={cn("border-t border-border", i % 2 === 0 ? "bg-muted/20" : "bg-muted/5")}
 >
...
function StructuresTab() {
 ...
 <div key={row.feat} className="grid grid-cols-3 gap-2 text-xs text-muted-foreground border-b border-border pb-2 last:border-0">
...
function MarketTab() {
 ...
 <tbody>
 {COUNTRIES.map((c, i) => (
 <tr key={c.country} className={cn("border-t border-border", i % 2 === 0 ? "bg-muted/20" : "bg-muted/5")}>
