...
function StatCard({
 ...
 <span className={`text-2xl font-semibold tabular-nums ${valClass}`}>
 {value}
 </span>
...
function InfrastructureTab() {
 ...
 {selected.regulated && (
 <Badge variant="outline" className="text-foreground border-border bg-muted/10">
 Regulated
 </Badge>
 )}
 </div>
 ...
 <p className="text-lg font-semibold text-emerald-400">{fmtPct(selected.yieldPct)}</p>
 </div>
 <div>
 <p className="text-xs text-muted-foreground">Total Return</p>
 <p className="text-lg font-medium text-foreground">{fmtPct(selected.totalReturn)}</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <td className="p-3 text-right text-foreground font-mono">{fmtPct(a.totalReturn)}</td>
...
function RealEstateTab() {
 ...
 className={
 r.type === "REIT"
 ? "text-foreground border-border bg-muted/10"
 : "text-amber-400 border-amber-400/30 bg-amber-400/10"
 }
 >
 {r.type}
 ...
 <td className="p-3 text-right font-mono text-foreground">{fmtPct(r.noiYield)}</td>
...
function TimberFarmlandTab() {
 ...
 <span className="text-foreground font-medium">{fmtPct(selected.totalReturn)}</span>
 </div>
 </div>
 </div>
 ...
 <p className="font-mono text-foreground">{fmtPct(selected.landAppreciation)}</p>
 </div>
 <div>
 ...
 <td className="p-3 text-right font-mono text-foreground">{fmtPct(item.landAppreciation)}</td>
...
function PortfolioRoleTab() {
 ...
 return (
 <div key={sc.label} className={`p-3 rounded-lg border transition-colors ${isMax ? "border-primary/40 bg-muted/5" : "border-border bg-muted/10"}`}>
 <div className="flex items-center justify-between mb-2">
 <div className="flex items-center gap-2">
 <span className="text-sm font-medium">{sc.label}</span>
 {isMax && (
 <Badge className="text-xs bg-muted/10 text-foreground border-primary/30">
 Best Sharpe
 </Badge>
 )}
 </div>
 ...
 <span>Real Assets: <span className="text-foreground font-mono">{sc.realAssets}%</span></span>
 </div>
 </div>
 ...
 <span className={`font-mono ${isMax ? "text-foreground font-medium" : "text-foreground"}`}>
 {sc.sharpe.toFixed(2)}
 </span>
 </div>
 <div className="h-2 rounded-full bg-muted/30 overflow-hidden">
