...
// ---------------------------------------------------------------------------
function InfoCard({ title, children }: { title: string; children: React.ReactNode; icon: React.ComponentType<{ className?: string }> }) {
...
function ContagionTab() {
 ...
 <strong className="text-foreground">Key Insight:</strong> Contagion is non-linear. Small shocks can cascade into systemic crises when
...
function GSIBTab() {
 ...
 <div className="text-xs font-semibold text-foreground mb-0.5">{cat.name}</div>
 <Badge variant="outline" className="text-xs border-border text-foreground mb-1">{cat.weight}</Badge>
...
export default function SystemicRiskPage() {
 ...
 { value: "metrics", label: "Systemic Risk Metrics" },
 { value: "macropru", label: "Macroprudential Tools" },
 ].map(({ value, label }) => (
 <TabsTrigger
 key={value}
 value={value}
