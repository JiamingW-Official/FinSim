...
const ACHIEVEMENT_ICON_MAP: Record<string, React.ReactElement> = {
...
const ARCHETYPE_ICON_MAP: Record<string, React.ElementType> = {
 Search,
 TrendingUp,
 DollarSign,
 Target,
...
export default function ProfilePage() {
 ...
 <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-foreground/5 border border-border text-muted-foreground">
 {(() => { const Icon = ARCHETYPE_ICON_MAP[archetype.icon] ?? Target; return <Icon className="h-5 w-5" />; })()}
 </div>
 ...
 className="flex h-9 w-9 items-center justify-center rounded-lg bg-foreground/5 border border-border text-muted-foreground"
 >
 {ACHIEVEMENT_ICON_MAP[a.icon] ?? <Target className="h-4 w-4" />}
 </div>
 ))}
 </div>
 )}
 ...
 { style: "Options", trades: stats.optionsTradesCount ?? 0, winRate: Math.max(0, winRate - 3), pnl: stats.optionsTotalPnL ?? 0 },
 ].map((row) => (
 <div key={row.style} className="flex items-center justify-between py-2 border-b border-border last:border-0">
 <div>
 ...
 <p className={`text-xs ${(row.pnl ?? 0) >= 0 ? "text-emerald-400" : "text-red-400"}`}>
 ...
 <div key={a.id} className="flex items-center gap-3 rounded-lg bg-card border border-border px-3 py-2.5">
 <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-foreground/5 border border-border text-muted-foreground shrink-0">
 {ACHIEVEMENT_ICON_MAP[a.icon] ?? <Target className="h-4 w-4" />}
 </div>
