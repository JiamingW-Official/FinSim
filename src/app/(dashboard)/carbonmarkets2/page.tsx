...
const SECTOR_COVERAGE = [
 ...
 { sector: "Aviation (EU)", icon: <Plane size={14} />, coverage: "~70%", freeAlloc: "25%", phase: "Phase 4", color: "text-foreground" },
 ...
 { sector: "Road Transport (ETS2)", icon: <TrendingUp size={14} />, coverage: "100%", freeAlloc: "0%", phase: "ETS2 2027", color: "text-foreground" },
];
...
function EuEtsTab() {
 ...
 <p className="text-foreground font-medium">How CBAM Works</p>
 ...
 <div className="bg-muted rounded-lg p-3 space-y-2">
 <p className="text-foreground font-medium">Covered Sectors (Phase-in)</p>
 ...
 { label: "Post-2023 Cancellation", val: "Above auction vol", dir: "Permanent", color: "text-foreground" },
 ].map((item) => (
...
function VoluntaryTab() {
 ...
 <th className="text-center py-2 pr-4 text-foreground">Verra VCS</th>
 ...
 <p className="text-xs text-foreground italic">&ldquo;{gc.claim}&rdquo;</p>
...
function NatureTab() {
 ...
 <p className="text-xl font-semibold text-foreground">{b.buffer}</p>
 ...
 <span className="text-xs font-mono text-foreground shrink-0 mt-0.5">{p.id}</span>
 <div>
...
function CorporateTab() {
 ...
 {co.sbtAligned ? (
 <Badge variant="outline" className="text-xs text-foreground border-border">SBTi</Badge>
 ) : (
 ...
 <p className={`text-lg font-semibold text-foreground mb-2`}>{tier.priceRange}</p>
 ...
 <Info size={12} className="text-foreground shrink-0" />
 )}
...
export default function CarbonMarkets2Page() {
 ...
 {[
 { label: "EU ETS Price", value: "€59.4/t", sub: "2024 average", color: "text-foreground" },
 { label: "VCM Volume", value: "~350 MtCO₂e", sub: "Issued 2023", color: "text-emerald-400" },
 { label: "CBAM Start", value: "Jan 2026", sub: "Financial obligations", color: "text-foreground" },
