...
function PortfolioStructureTab() {
 ...
 <span className="text-xs text-muted-foreground ">{rb.label}</span>
 <div className="h-2 bg-muted rounded-full">
 <div className={`h-full rounded-full ${rb.color}`} style={{ width: `${rb.pct}%` }} />
 </div>
 <span className="text-sm font-semibold text-foreground">{rb.pct}%</span>
 </div>
 ))}
 </div>
 </CardContent>
...
function ShortSellingTab() {
 ...
 <div className="w-7 h-7 rounded-full bg-sky-500/20 border border-sky-500/40 flex items-center justify-center text-[11px] font-semibold text-sky-400">
 {item.step}
 </div>
 {idx < 5 && (
...
export default function EquityLongShortPage() {
 ...
 { value: "pairs", label: "Pairs Trading" },
