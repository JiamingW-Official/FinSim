        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {inflationRegimes.map((r) => (
            <div
              key={r.label}
              className={cn(
                "flex flex-col items-center px-3 py-2 rounded-lg border text-xs transition-all",
                r.color,
                r.active ? "ring-1 ring-current shadow-sm" : "opacity-60"
              )}
            >
                ...
                >
                  ...
                  {HEDGING_MATRIX[si].map((score, ai) => (
                    <td key={ai} className="py-2 px-2 text-center">
                      <span className={cn("inline-block w-7 h-7 rounded text-xs font-bold flex items-center justify-center border", heatColor(score))}>
                        {score}
                      </span>
                    </td>
                  ))}
...
export default function InflationLinkedPage() {
  return (
    <div className="min-h-screen bg-[#0a0f1a] text-white p-4 sm:p-6">
