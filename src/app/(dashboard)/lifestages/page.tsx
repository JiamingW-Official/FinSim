...
function ChecklistCard({ items }: { items: ChecklistItem[] }) {
 ...
 checked.has(item.id) ? "bg-muted/10" : "hover:bg-muted/50"
 )}
 ...
 checked.has(item.id) ? "text-foreground" : "text-muted-foreground/40"
 )}
 />
 <div className="min-w-0">
...
function Hint({ text }: { text: string }) {
 return (
 <div className="flex items-start gap-2 p-3 rounded-lg bg-muted/10 border border-border text-xs text-foreground">
 <span>{text}</span>
 </div>
 );
}
...
function Tab50s() {
 ...
 <span className="text-xs font-semibold text-emerald-400">{row.age}</span>
 </div>
...
// ─────────────────────────────────────────────────────────────────────────────
const TABS = [
 { value: "20s", label: "20s", subtitle: "Foundation" },
 { value: "30s", label: "30s", subtitle: "Accumulation" },
 { value: "40s", label: "40s", subtitle: "Peak Earning" },
 { value: "50s", label: "50s", subtitle: "Pre-Retirement" },
 { value: "60s", label: "60s+", subtitle: "Distribution" },
];
