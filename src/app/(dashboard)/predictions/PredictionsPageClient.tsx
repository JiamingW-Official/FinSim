...
function MarketRow({
 ...
 <button
   ...
   className="group w-full text-left border-b border-border last:border-b-0 px-4 py-4 hover:bg-foreground/[0.02] transition-colors duration-150"
 >
   <div className="flex items-start gap-4">
     {/* LEFT: category + question + bar */}
     <div className="flex-1 min-w-0">
       <div className="flex items-center gap-1.5 mb-2">
         ...
         {isUrgent && (
           <span className="inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-medium text-amber-500 bg-amber-500/10">
         ...
         {bet && (
           <span className={cn(
             ...
             bet.position === "yes" ? "text-emerald-500 bg-emerald-500/10" : "text-red-500 bg-red-500/10"
           )}>
             {bet.position}
           </span>
         )}
       ...
       <div className="flex items-center gap-2">
         ...
         <span className="shrink-0 text-right font-mono tabular-nums text-[10px] text-muted-foreground/40">
           NO {100 - prob}%
         </span>
       </div>
     </div>
...
function PredictorTips() {
 ...
 <h2 className="text-base font-serif tracking-tight text-foreground">How to Be a Good Predictor</h2>
...
export function PredictionsPageClient() {
 ...
 className={cn(
 "flex shrink-0 items-center gap-1.5 border-b-2 px-3 py-2.5 text-[11px] transition-colors",
 pageTab === tab.value
 ? "border-foreground text-foreground font-semibold"
 : "border-transparent text-muted-foreground/50 hover:text-foreground font-medium",
 )}
 >
 {tab.label}
 ...
 <span className="text-xs font-serif tabular-nums font-medium">
 ...
 <span className="font-serif text-3xl font-bold tabular-nums leading-none">{insightPoints.toLocaleString()}</span>
 <span className="text-[11px] text-muted-foreground/40 mb-0.5 font-mono">pts</span>
 </div>
 <p className="text-[10px] font-mono text-muted-foreground/30 uppercase tracking-wide mt-0.5">Available</p>
 </div>
 </div>
 </div>
 )}
 ...
 className="w-full rounded-md border border-border bg-transparent py-1.5 pl-8 pr-8 text-[11px] text-foreground placeholder-muted-foreground/40 outline-none focus:border-border transition-colors"
 />
 {searchQuery && (
 ...
 >
 {tab.value !== "all" && (
 <span className={cn("inline-block h-1.5 w-1.5 rounded-full", CATEGORY_DOT_COLORS[tab.value] ?? "bg-muted-foreground")} />
 )}
 {tab.label}
 </button>
 ))}
 ...
 className="appearance-none rounded border border-border bg-transparent px-2 py-1 text-xs font-medium text-foreground outline-none"
 >
 ...
 <div>
 <h2 className="text-base font-serif tracking-tight text-foreground">Kelly Criterion Calculator</h2>
 ...
 <div>
 <h2 className="text-base font-serif tracking-tight text-foreground">Market Depth</h2>
 <p className="mt-0.5 text-[11px] text-muted-foreground">
 ...
 <div>
 <h2 className="text-base font-serif tracking-tight text-foreground">Calibration Chart</h2>
 <p className="mt-0.5 text-[11px] text-muted-foreground">
