...
function DeltaHedgingTab() {
 ...
 <div className="text-foreground font-medium mb-1">Delta = Hedge Ratio</div>
 ...
 <div className="bg-muted/10 border border-border rounded-lg p-3">
 <div className="text-foreground font-medium mb-1">Hedge P&amp;L Formula</div>
...
function GammaPnLTab() {
 ...
 <div className="text-foreground border-t border-border pt-1 mt-1">Net = Gamma − |Theta|</div>
 </div>
...
function VolArbitrageTab() {
 ...
 const bg = iv > 0.28 ? "bg-red-500/20 text-red-300" : iv > 0.25 ? "bg-amber-500/20 text-amber-300" : "bg-muted/10 text-foreground";
 return (
...
function MMRiskTab() {
 ...
 <div className={`text-lg font-semibold text-${card.color}-400 font-mono mt-1`}>{card.value}</div>
 ...
 <span className="flex items-center gap-1.5 text-foreground"><span className="w-4 h-0.5 bg-primary inline-block" /> Net P&amp;L</span>
 </div>
 </div>
...
export default function GammaScalpingPage() {
 ...
 { value: "vol", label: "Vol Arbitrage" },
 { value: "mm", label: "MM Risk Mgmt" },
 ].map(({ value, label }) => (
 <TabsTrigger
 key={value}
 value={value}
