        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.4 }}>
          <Tabs defaultValue="library">
            <TabsList className="bg-muted/50 mb-4">
              <TabsTrigger value="library">Signal Library</TabsTrigger>
              <TabsTrigger value="backtest">Backtesting</TabsTrigger>
              <TabsTrigger value="decay">Alpha Decay</TabsTrigger>
              <TabsTrigger value="zoo">Factor Zoo</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="library" className="space-y-4">
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {filteredSignals.map((sig, i) => {
                          ...
                          return (
                            <tr key={sig.id} className={`border-b border-border/50 hover:bg-muted/20 transition-colors ${i % 2 === 0 ? "" : "bg-muted/10"}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { label: "IC (Information Coefficient)", desc: "Rank correlation between signal values and subsequent returns. IC > 0.05 is considered strong; 0.02–0.05 is typical for well-constructed signals.", icon: Activity, color: "#6366f1" },
                  { label: "IR (Information Ratio)", desc: "IC mean divided by IC standard deviation (IC_IR = μ_IC / σ_IC). Analogous to a Sharpe ratio for signal quality. IR > 0.5 indicates a robust signal.", icon: Target, color: "#22c55e" },
                  { label: "Decay Half-Life", desc: "The time it takes for a signal's predictive IC to fall to 50% of its peak. Momentum signals decay in days; value signals may persist for months.", icon: Clock, color: "#f59e0b" },
                ].map((item) => (
            ...
            <TabsContent value="backtest" className="space-y-4">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                {[
                  { label: "L/S Annual Return", value: "+14.2%", sub: "Top vs Bottom quintile",  color: "#6366f1", icon: TrendingUp   },
                  { label: "Sharpe Ratio",       value: "1.74",  sub: "Annualized L/S portfolio", color: "#22c55e", icon: Target      },
                  { label: "Max Drawdown",        value: "-8.3%", sub: "Worst peak-to-trough",    color: "#ef4444", icon: TrendingDown },
                ].map((m) => (
                  <Card key={m.label} className="border-border bg-card">
                    <CardContent className="pt-4 pb-3 flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: m.color + "22" }}>
            ...
            <TabsContent value="zoo" className="space-y-4">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: "Published Factors",    value: "400+", sub: "Academic literature",      color: "#6366f1", icon: Layers     },
                  { label: "In Sample Set",         value: String(FACTOR_ZOO.length), sub: "Representative sample", color: "#22c55e", icon: BarChart3  },
                  { label: "Out-of-Sample Survive", value: String(replicatedCount), sub: `${((replicatedCount/FACTOR_ZOO.length)*100).toFixed(0)}% replication rate`, color: "#f59e0b", icon: CheckCircle },
                  { label: "Failed Replication",    value: String(notReplicated), sub: "Data mining artifacts",    color: "#ef4444", icon: AlertTriangle },
                ].map((m) => (
                  <Card key={m.label} className="border-border bg-card">
                    <CardContent className="pt-4 pb-3 flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: m.color + "22" }}>
                ...
                <CardContent className="p-0">
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {FACTOR_ZOO.map((f, i) => (
                          <tr key={f.name} className={`border-b border-border/50 hover:bg-muted/20 transition-colors ${i % 2 === 0 ? "" : "bg-muted/10"}`}>
