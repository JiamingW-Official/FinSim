...
function Tab2DigitalScoring() {
 ...
 { factor: "Credit Mix", weight: 10, color: "bg-muted/10" },
 { factor: "New Credit Inquiries", weight: 10, color: "bg-muted/10" },
 ].map((f) => (
...
function Tab3EmbeddedFinance() {
 ...
 <div>
 <span className="text-sm font-semibold text-foreground">{cs.company}</span>
...
function Tab4RiskRegulation() {
 ...
 const severityColor = item.severity === "high" ? "text-rose-400 border-rose-500/30 bg-rose-500/10" : item.severity === "medium" ? "text-amber-400 border-amber-500/30 bg-amber-500/10" : "text-foreground border-border bg-muted/10";
 return (
...
export default function BNPLLendingPage() {
 ...
 {[
 { label: "Global BNPL Market", val: "$450B GMV", color: "bg-muted/10 text-foreground border-border" },
 ...
 { label: "Embedded Finance", val: "$124B Market", color: "bg-muted/10 text-foreground border-border" },
 ].map((chip) => (
