      ...
      <div className="relative z-10 flex items-center w-full gap-1 text-[10px] font-mono tabular-nums">
        {isBid ? (
          <>
            ...
            <span className="flex-1 text-right text-emerald-500 font-medium">
      ...
      <div className="flex">
        ...
        <div className="w-1/2 border-r border-border/30">
          {book.bids.map((level, i) => (
            <LevelRow key={i} level={level} maxSize={maxSize} side="bid" />
          ))}
        </div>
        {/* Asks column */}
        <div className="w-1/2">
          {book.asks.map((level, i) => (
            <LevelRow key={i} level={level} maxSize={maxSize} side="ask" />
          ))}
        </div>
      </div>
