          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {folds.map((fold) => {
                ...
                return (
                  <tr key={fold.foldIndex} className="border-b border-border/20 hover:bg-muted/30">
...
function TimelineBar({ folds, totalBars }: { folds: WalkForwardFold[]; totalBars: number }) {
  ...
  return (
    <div className="flex h-8 w-full overflow-hidden rounded-lg border border-border/20">
      {folds.map((fold) => {
...
function MetricChip({ label, value, positive }: { label: string; value: string; positive?: boolean }) {
  return (
    <div className="rounded-lg border border-border/20 bg-muted/15 px-3 py-2">
