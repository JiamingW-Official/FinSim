...
function LabMetric({ label, value, positive, sub }: { label: string; value: string; positive?: boolean; sub?: string }) {
  return (
    <div className="flex flex-col gap-0.5 rounded-md border border-border/20 bg-card/60 px-3 py-2 min-w-[100px]">
...
function ConditionRow({
  ...
  return (
    <div className="flex items-center gap-2 rounded-md border border-border/20 bg-card/60 px-3 py-2">
      ...
      <button onClick={onRemove} className="text-muted-foreground/70 hover:text-red-400 transition-colors">
...
export default function StrategyLab() {
  ...
  return (
    <div className="space-y-0">
      {/* Sub-tab bar */}
      <div className="flex gap-0 border-b border-border/20 bg-black/10 overflow-x-auto shrink-0">
        {LAB_SECTIONS.map(s => (
      ...
      {section === "builder" && (
        <div className="p-6 space-y-6">
          {/* Preset pills */}
          <div>
            <p className="mb-2 text-xs font-semibold text-muted-foreground">Preset Strategies</p>
            <div className="flex flex-wrap gap-2">
              {PRESETS.map(p => (
                <button key={p.id} onClick={() => handleLoadPreset(p)}
                  className="rounded-full border border-border/20 bg-card px-3 py-1 text-xs text-muted-foreground hover:border-primary/50 hover:text-primary transition-colors"
                  title={p.description}
                >
                  {p.name}
                </button>
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              <label className="mb-1 block text-xs font-semibold text-muted-foreground">Strategy Name</label>
              <input
                value={strategy.name}
                onChange={e => setStrategy(s => ({ ...s, name: e.target.value }))}
                className="w-full max-w-xs rounded-md border border-border/20 bg-card px-3 py-1.5 text-sm text-foreground outline-none focus:ring-1 focus:ring-violet-500"
              />
            </div>
            <Button
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <Card className="border-border/20 bg-card/50 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-semibold text-foreground">Entry Rules</h3>
              ...
              {strategy.entryConditions.length < 3 && (
                <button onClick={addEntry} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-muted-foreground transition-colors">
            ...
            <Card className="border-border/20 bg-card/50 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-semibold text-foreground">Exit Rules</h3>
              ...
              {strategy.exitConditions.length < 2 && (
                <button onClick={addExit} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-muted-foreground transition-colors">
                  <Plus className="h-3 w-3" /> Add condition
                </button>
              )}
              <div className="text-xs text-muted-foreground/70 flex items-center gap-1">
            ...
            <Card className="border-border/20 bg-card/50 p-4 space-y-4">
              <h3 className="text-xs font-semibold text-foreground">Sizing & Risk</h3>
              <div className="space-y-2">
                {([
              ...
              {strategy.sizing !== "kelly" && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-md border border-border/20 bg-muted px-3 py-1.5 text-sm text-foreground outline-none focus:ring-1 focus:ring-violet-500"
                  />
                </div>
              )}
              <div className="space-y-2 pt-1 border-t border-border/20">
                <p className="text-xs font-semibold text-muted-foreground">Risk Rules</p>
                {[
                  ...
                  <div key={key}>
                    ...
                    <input
                      ...
                      className="w-full rounded border border-border/20 bg-muted px-2 py-1 text-xs text-foreground outline-none focus:ring-1 focus:ring-violet-500"
                    />
                  </div>
                ))}
              </div>
          ...
          <Card className="border-border/20 bg-card/50 p-4">
            <p className="text-xs font-semibold text-muted-foreground mb-1.5">Strategy Logic</p>
        ...
        <div className="p-6 space-y-6">
          {!result ? (
            ...
            <>
              {/* Metrics grid */}
              <div>
                <p className="mb-3 text-xs font-semibold text-muted-foreground">Performance Metrics</p>
                <div className="flex flex-wrap gap-2">
              ...
              <Card className="border-border/20 bg-card/50 p-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-xs font-semibold text-muted-foreground">Equity Curve</h3>
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1.5"><span className="inline-block h-0.5 w-4 bg-primary" /> Strategy</span>
                    <span className="flex items-center gap-1.5"><span className="inline-block h-0.5 w-4 bg-muted" style={{ borderTop: "1px dashed" }} /> B&H</span>
                    ...
                    <tbody>
                      {result.trades.slice(0, 20).map((t, i) => (
                        <tr key={i} className="border-b border-border/20 hover:bg-muted/30">
                          ...
                          <td className="py-1.5 pr-3 text-muted-foreground">{t.exitBar}</td>
                          <td className="py-1.5 pr-3 text-muted-foreground">${t.exitPrice.toFixed(2)}</td>
                          <td className="py-1.5 pr-3 text-muted-foreground">{t.holdingBars}b</td>
                      ...
                      {result.trades.length === 0 && (
                        <tr><td colSpan={8} className="py-4 text-center text-muted-foreground/70">No trades generated</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
      ...
      {section === "optimization" && (
        <div className="p-6 space-y-6">
          {/* Controls */}
          <Card className="border-border/20 bg-card/50 p-4 space-y-4">
            <h3 className="text-xs font-semibold text-foreground">Parameter Sweep (5×5 = 25 combinations)</h3>
            <div className="grid grid-cols-3 gap-4 items-end">
              <div>
                ...
                <select value={optParamA} onChange={e => setOptParamA(e.target.value as typeof optParamA)}
                  className="w-full rounded-md border border-border/20 bg-muted px-3 py-1.5 text-xs text-muted-foreground outline-none">
          ...
          <Card className="border-amber-500/20 bg-amber-500/5 p-4">
            <div className="flex items-start gap-2">
              ...
              <div>
                <h4 className="text-xs font-semibold text-amber-300">Overfitting Warning</h4>
          ...
          {/* Walk-forward 70/30 */}
          <Card className="border-border/20 bg-card/50 p-4 space-y-4">
            <h3 className="text-xs font-semibold text-foreground">Walk-Forward Analysis — 70% Train / 30% Out-of-Sample</h3>
            <div className="grid grid-cols-2 gap-4">
              {[
                { label: "In-Sample (700 bars)", r: wfInSample },
                { label: "Out-of-Sample (300 bars)", r: wfOutSample },
              ].map(({ label, r }) => (
                <div key={label} className="rounded-md border border-border/20 bg-card/60 p-3 space-y-1.5">
                  ...
                  {[
                    ...
                    <div key={l} className="flex justify-between text-xs">
                      ...
                      <span className={cn("font-semibold", pos === true ? "text-green-400" : pos === false ? "text-red-400" : "text-muted-foreground")}>{v}</span>
                    </div>
                  ))}
                </div>
              ))}
      ...
      {section === "montecarlo" && (
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-semibold text-foreground">Monte Carlo Analysis</h3>
              <p className="text-xs text-muted-foreground mt-0.5">500 simulations by shuffling trade returns. Requires backtest results.</p>
            </div>
            <Button
              onClick={handleRunMC}
          ...
          {!result && (
            <Card className="border-border/20 bg-card/40 p-8 text-center">
          ...
          {mcSimulations && mcFinalValues && mcRisk && (
            <>
              {/* Fan chart */}
              <Card className="border-border/20 bg-card/50 p-4">
                <h3 className="mb-3 text-xs font-semibold text-muted-foreground">Equity Fan Chart — 500 Paths</h3>
                <MonteCarloFan simulations={mcSimulations} buyAndHold={result?.buyAndHold ?? 0} />
              </Card>

              {/* Risk metrics */}
              <div className="grid grid-cols-3 gap-4">
                <Card className="border-border/20 bg-card/50 p-4 text-center space-y-1">
                  <p className="text-xs text-muted-foreground">Prob. of Positive Returns</p>
                  <p className={cn("text-2xl font-semibold tabular-nums", mcRisk.pctPositive >= 0.5 ? "text-green-400" : "text-red-400")}>
                    {(mcRisk.pctPositive * 100).toFixed(1)}%
                  </p>
                  <p className="text-xs text-muted-foreground/70">of 500 paths end profitable</p>
                </Card>
                <Card className="border-border/20 bg-card/50 p-4 text-center space-y-1">
                  <p className="text-xs text-muted-foreground">Worst-Case P5 (5th pct)</p>
                  <p className={cn("text-2xl font-semibold tabular-nums", mcRisk.worstP5 >= 10000 ? "text-green-400" : "text-red-400")}>
                    ${mcRisk.worstP5.toFixed(0)}
                  </p>
                  <p className="text-xs text-muted-foreground/70">on $10,000 start</p>
                </Card>
                <Card className="border-border/20 bg-card/50 p-4 text-center space-y-1">
                  <p className="text-xs text-muted-foreground">Prob. of 20%+ Drawdown</p>
                  <p className={cn("text-2xl font-semibold tabular-nums", mcRisk.pct20dd <= 0.2 ? "text-green-400" : "text-red-400")}>
                    {(mcRisk.pct20dd * 100).toFixed(1)}%
                  </p>
                  <p className="text-xs text-muted-foreground/70">equity falls below $8,000</p>
                </Card>
              </div>

              {/* Final value histogram */}
              <Card className="border-border/20 bg-card/50 p-4">
                <h3 className="mb-3 text-xs font-semibold text-muted-foreground">Distribution of Final Portfolio Values</h3>
      ...
      {section === "paper" && (
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-semibold text-foreground">Live Paper Trading</h3>
              <p className="text-xs text-muted-foreground mt-0.5">Synthetic signals refresh every 5 seconds. Not real data.</p>
            </div>
          ...
          <Card className="border-border/20 bg-card/50 p-4">
            <h3 className="mb-3 text-xs font-semibold text-muted-foreground">Recent Signals (last 20)</h3>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border/20 text-left text-muted-foreground">
                  ...
                  {liveSignals.map((sig, i) => (
                    <tr key={sig.id} className="border-b border-border/20 hover:bg-muted/30">
                      <td className="py-1.5 pr-4">
                        <span className="font-semibold text-foreground">{sig.ticker}</span>
                      </td>
                      <td className="py-1.5 pr-4">
                        <Badge className="text-[11px] bg-primary/15 text-primary">{sig.signalType}</Badge>
                      </td>
                      <td className="py-1.5 pr-4 text-muted-foreground">{sig.condition}</td>
                      <td className="py-1.5 pr-4 text-muted-foreground">{sig.time}</td>
                      <td className="py-1.5 font-semibold text-foreground">${sig.suggestedEntry}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {attribution && (
            <div className="grid grid-cols-3 gap-4">
              <Card className="border-border/20 bg-card/50 p-4 text-center">
                <p className="text-xs text-muted-foreground mb-1">Strategy P&L (20d)</p>
                <p className={cn("text-2xl font-semibold tabular-nums", attribution.strategyTotal >= 0 ? "text-green-400" : "text-red-400")}>
                  {attribution.strategyTotal >= 0 ? "+" : ""}{(attribution.strategyTotal * 100).toFixed(2)}%
                </p>
              </Card>
              <Card className="border-border/20 bg-card/50 p-4 text-center">
                <p className="text-xs text-muted-foreground mb-1">SPY Benchmark (20d)</p>
                <p className={cn("text-2xl font-semibold tabular-nums", attribution.benchmarkTotal >= 0 ? "text-muted-foreground" : "text-red-400")}>
              ...
              <Card className={cn("p-4 text-center", attribution.alpha >= 0 ? "border-green-500/20 bg-green-500/5" : "border-red-500/20 bg-red-500/5")}>
                <p className="text-xs text-muted-foreground mb-1">Alpha (vs SPY)</p>
                <p className={cn("text-2xl font-semibold tabular-nums", attribution.alpha >= 0 ? "text-green-400" : "text-red-400")}>
          ...
          {dailyPnL.length > 0 && (
            <Card className="border-border/20 bg-card/50 p-4">
              <h3 className="mb-3 text-xs font-semibold text-muted-foreground">Daily P&L — Last 20 Days (Strategy vs SPY)</h3>
