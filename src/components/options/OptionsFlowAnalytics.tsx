...
function ActivityFeed({ entries }: { entries: FlowEntry[] }) {
 ...
 {/* Summary bar */}
 <div className="mx-4 mt-3 rounded-md border border-border bg-card/40 p-3">
...
function FlowHeatmapSection({ entries }: { entries: FlowEntry[] }) {
 ...
 <div className="flex-1 rounded-md border border-border bg-card/40 p-2">
...
function DarkPoolCorrelation({ seed }: { seed: number }) {
 ...
 return (
 <tr key={c.ticker} className="border-b border-border transition-colors hover:bg-muted/50">
...
export default function OptionsFlowAnalytics() {
 ...
 return (
 <div className="flex h-full flex-col overflow-hidden rounded-md border border-border bg-card text-foreground">
 {/* Header */}
 ...
 <button
 onClick={refresh}
 className="rounded border border-border bg-muted/20 px-2 py-1 text-[11px] text-muted-foreground transition-colors hover:text-foreground"
 >
 Refresh
 </button>
 </div>
 ...
 {/* Tab bar */}
 <div className="flex shrink-0 items-center gap-0.5 border-b border-border px-4 py-1.5">
 {TABS.map(({ key, label }) => (
 <button
 key={key}
 onClick={() => setTab(key)}
