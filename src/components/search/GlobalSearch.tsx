...
function GlobalSearchModal({ open, onClose }: { open: boolean; onClose: () => void }) {
 ...
 <kbd className="hidden sm:inline-flex items-center rounded border border-border bg-muted/50 px-1.5 py-0.5 text-xs font-mono text-muted-foreground">
 ESC
 </kbd>
 </div>
 </div>
 ...
 type="button"
 onClick={() => setQuery(s)}
 className="rounded-full border border-border bg-muted/30 px-2.5 py-0.5 text-xs text-muted-foreground hover:border-border hover:text-foreground transition-colors"
 >
 {s}
 </button>
 ))}
 ...
 <kbd className="rounded border border-border bg-muted/40 px-1 py-px font-mono">↑</kbd>
 <kbd className="rounded border border-border bg-muted/40 px-1 py-px font-mono">↓</kbd>
 navigate
 </span>
 <span className="flex items-center gap-1">
