...
function PerformanceAttribution() {
 ...
 <div key={s.label} className="rounded-md border border-border bg-background/60 px-3 py-2">
 ...
 <tbody>
 {rows.map((r) => (
 <tr key={r.name} className="border-b border-border hover:bg-muted/10 transition-colors">
...
function RiskAdjustedSection() {
 ...
 <tbody>
 {rows.map((r) => (
 <tr key={r.label} className="border-b border-border hover:bg-muted/10 transition-colors">
...
function FactorAnalysisSection() {
 ...
 <tbody>
 {loadings.map((f) => (
 <tr key={f.label} className="border-b border-border hover:bg-muted/10 transition-colors">
...
function DrawdownSection() {
 ...
 <div key={s.label} className="rounded-md border border-border bg-background/60 px-2.5 py-2">
 <div className="text-[11px] text-muted-foreground">{s.label}</div>
 <div className={cn("text-sm font-semibold tabular-nums mt-0.5", s.color)}>{s.value}</div>
 </div>
 ))}
...
function TailRiskSection() {
 ...
 return (
 <tr key={c.name} className="border-b border-border hover:bg-muted/10 transition-colors">
...
function StyleBox({ styleIndex }: { styleIndex: number }) {
 ...
 isActive ? "bg-primary/30 border-primary/60 text-primary font-semibold" : "border-border bg-muted/10 text-muted-foreground/40"
 )}
 title={`${r} / ${c}`}
 >
 {isActive ? `${r[0]}/${c[0]}` : ""}
...
function PeerComparisonSection() {
 ...
 <tbody>
 {metrics.map((m) => (
 <tr key={m.label} className="border-b border-border hover:bg-muted/10 transition-colors">
 ...
 <div className="flex items-start gap-6 p-3 rounded-md border border-border bg-background/60">
...
export default function AdvancedAnalytics() {
 ...
 activeTab === tab.id
 ? cn("bg-card border border-border", tab.color)
