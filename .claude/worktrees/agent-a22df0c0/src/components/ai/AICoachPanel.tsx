...
import { ChevronDown, ChevronUp, RotateCcw, AlertCircle, Zap, Brain, X } from "lucide-react";
...
import {
  ...
  type WeeklyReport,
  type RadarAxis,
} from "@/services/ai/behavioral-coach";

type Mode = "trade" | "review" | "brief" | "behavior";
...
const MODES: { value: Mode; label: string; desc: string }[] = [
  ...
  { value: "behavior", label: "Behavior", desc: "Behavioral bias analysis" },
];
...
// ─── Behavioral Panel Sub-Components ─────────────────────────────────────────
    ...
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} aria-label="Behavioral radar chart">
      ...
      {gridLevels.map((lvl) => {
        const pts = Array.from({ length: n }, (_, i) => {
          ...
          const p = pointAtAngle(angle, r * lvl);
          return `${p.x},${p.y}`;
        }).join(" ");
        return (
          <polygon
      ...
      {axes.map((_, i) => {
        const angle = (i / n) * 2 * Math.PI;
        const outer = pointAtAngle(angle, r);
        return (
          <line
            key={i}
            x1={cx}
      ...
      {(() => {
        const pts = axes.map((ax, i) => {
          const angle = (i / n) * 2 * Math.PI;
          const p = pointAtAngle(angle, r * (ax.value / 100));
          return `${p.x},${p.y}`;
        }).join(" ");
        return (
          <polygon
      ...
      {axes.map((ax, i) => {
        const angle = (i / n) * 2 * Math.PI;
        const labelR = r + 14;
        const p = pointAtAngle(angle, labelR);
        return (
          <text
            key={i}
            x={p.x}
...
function ScoreBar({
  ...
  // inverse=true: low is good (scores like disposition effect)
...
function BehaviorPanel({ trades, accountValue }: { trades: import("@/types/trading").TradeRecord[]; accountValue: number }) {
  const report: WeeklyReport = useMemo(
    () => generatePostSessionReport(trades, accountValue),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [trades.length, accountValue],
  );
  ...
  return (
    <div className="space-y-3">
      ...
      {report.behavioralRegressions.length > 0 && (
        <div className="space-y-1">
          <div className="text-[9px] font-bold text-amber-400 uppercase tracking-wider">
            Patterns to watch
          </div>
          {report.behavioralRegressions.map((r, i) => (
            <div key={i} className="flex gap-1.5 text-[9px] leading-tight">
      ...
      {/* Improvement targets */}
      {report.improvementTargets.length > 0 && (
        <div className="rounded-md border border-primary/20 bg-primary/5 px-2 py-2 space-y-1">
          <div className="text-[9px] font-black text-primary uppercase tracking-wider">
            This week: focus on
          </div>
          {report.improvementTargets.map((t, i) => (
            <div key={i} className="flex gap-1.5 text-[9px] leading-tight">
      ...
      {trades.length < 5 && (
        <div className="text-[9px] text-muted-foreground/60 text-center italic">
          Make more trades to unlock deeper behavioral analysis.
        </div>
      )}
    </div>
  );
