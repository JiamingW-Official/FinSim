    ...
    >
      ...
      <div className="px-3 py-2 border-b border-[#1e293b]">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {(
            [
              { label: "Stock +5%", stockChg: 0.05, ivChgPts: 0, daysFwd: 0 },
              { label: "Stock −5%", stockChg: -0.05, ivChgPts: 0, daysFwd: 0 },
              { label: "IV +10pts", stockChg: 0, ivChgPts: 10, daysFwd: 0 },
              { label: "Time −7d", stockChg: 0, ivChgPts: 0, daysFwd: 7 },
            ] as { label: string; stockChg: number; ivChgPts: number; daysFwd: number }[]
          ).map((sc) => {
            const priceChange = spotPrice * sc.stockChg;
            const deltaNew = greeks.delta + greeks.gamma * priceChange;
            // Vega is unchanged for stock-price moves; IV change doesn't affect vega itself
            const vegaNew = greeks.vega;
            // Theta accelerates slightly when near expiry / large stock move
            const thetaNew =
              greeks.theta * (1 + (Math.abs(sc.stockChg) / 1) * 0.5);
            const deltaDiff = deltaNew - greeks.delta;
            const thetaDiff = thetaNew - greeks.theta;
            const isStockUp = sc.stockChg > 0;
            const isStockDown = sc.stockChg < 0;
            const isIVUp = sc.ivChgPts > 0;
            const isTimeDecay = sc.daysFwd > 0;
            const accentColor = isStockUp
            ...
            return (
              <div
                key={sc.label}
                className={`rounded border p-1.5 ${accentColor}`}
              >
                <p className="text-[8px] font-semibold text-foreground mb-1">{sc.label}</p>
                <div className="space-y-0.5">
                  <div className="flex justify-between">
                    <span className="text-[8px] text-muted-foreground">Δ</span>
                    <span className="text-[8px] font-mono text-foreground">
                      {deltaNew.toFixed(3)}
                      <span
                        className={cn(
                          "ml-0.5",
                          deltaDiff > 0 ? "text-green-400" : deltaDiff < 0 ? "text-red-400" : "text-muted-foreground",
                        )}
                      >
                        ({deltaDiff >= 0 ? "+" : ""}{deltaDiff.toFixed(3)})
                      </span>
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {(isStockUp || isStockDown || isTimeDecay) && (
                    <div className="flex justify-between">
                      <span className="text-[8px] text-muted-foreground">Θ</span>
                      <span className="text-[8px] font-mono text-foreground">
                        {thetaNew.toFixed(3)}
                        <span
                          className={cn(
                            "ml-0.5",
                            thetaDiff < 0 ? "text-red-400" : "text-green-400",
                          )}
                        >
                          ({thetaDiff >= 0 ? "+" : ""}{thetaDiff.toFixed(3)})
                        </span>
                      </span>
                    </div>
                  )}
                  {isIVUp && (
                    <div className="flex justify-between">
                      <span className="text-[8px] text-muted-foreground">+10 IV</span>
                      <span className="text-[8px] font-mono text-amber-400">
                        +${(greeks.vega * 10).toFixed(2)}
                      </span>
                    </div>
                  )}
                </div>
