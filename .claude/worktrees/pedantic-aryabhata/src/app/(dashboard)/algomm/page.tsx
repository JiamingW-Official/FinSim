...
function Tab4PnlAttribution() {
 ...
 <div className="rounded-lg border border-border overflow-hidden">
 <p className="text-xs font-medium text-muted-foreground p-3 border-b border-border">
 Daily Breakdown
 </p>
 ...
 <tbody>
 {data.map((d) => (
 <tr key={d.day} className="border-b border-border hover:bg-muted/10">
...
function Tab5RegulationRisk() {
 ...
 <tr
 key={cb.type}
 className={cn("border-b border-border", i % 2 === 0 ? "bg-background/30" : "")}
 >
 <td className="p-2.5">
 <Badge
 variant="outline"
...
export default function AlgoMMPage() {
 ...
 <div
 key={kpi.label}
 className="flex items-center gap-1.5 rounded-full border border-border bg-muted/30 px-3 py-1 text-xs text-muted-foreground"
 >
