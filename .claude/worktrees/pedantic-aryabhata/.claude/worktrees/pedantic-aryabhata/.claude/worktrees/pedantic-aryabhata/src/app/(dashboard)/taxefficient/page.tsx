          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={bucket.label} className="rounded-lg p-3 space-y-2" style={{ backgroundColor: bucket.color + "10", border: `1px solid ${bucket.color}30` }}>
                ...
                <p className="text-xs text-muted-foreground/70 italic border-t border-border pt-2">{bucket.note}</p>
              </div>
            ))}
          </div>
        </CardContent>
        ...
        <Card className="border-amber-500/20">
          ...
          <CardContent className="space-y-1">
            {IRMAA_THRESHOLDS.slice(0, 4).map((t, i) => (
              <div key={i} className={cn("flex justify-between text-xs py-1 border-b border-border last:border-0", totalIncome > (IRMAA_THRESHOLDS[i - 1]?.income ?? 0) && totalIncome <= t.income && "text-amber-400 font-medium")}>
