...
function SignalBadge({ signal }: { signal: MeanReversionSignal["signal"] }) {
  const config = {
    strong_buy: { text: "Strong Buy", className: "bg-green-500/10 text-green-500" },
    buy: { text: "Buy", className: "bg-green-500/10 text-green-500" },
    ...
    <div className="flex items-center gap-1.5">
      ...
      <span
        className={cn(
          ...
          zScore < -1 ? "text-green-500" : zScore > 1 ? "text-red-500" : "text-muted-foreground",
        )}
      >
        {zScore >= 0 ? "+" : ""}
        {zScore.toFixed(1)}
