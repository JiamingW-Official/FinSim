...
export function SeasonBanner({ expanded, onToggle }: SeasonBannerProps) {
  ...
  return (
    <motion.button
      ...
      className="w-full rounded-lg border border-amber-500/20 bg-gradient-to-r from-amber-500/5 via-transparent to-amber-500/5 p-3 text-left transition-colors hover:bg-amber-500/[0.07]"
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className={cn(
              "text-[10px] font-bold",
              currentTier >= 20 ? "text-amber-400" : "text-muted-foreground/60",
            )}>
              Tier {currentTier}/20
            </span>
          </div>
          ...
          <div className="mt-1 flex items-center gap-2 text-[10px]">
            ...
            {nextTier && (
              <span className="text-muted-foreground/60">/ {nextTier.required} for Tier {nextTier.tierNumber}</span>
            )}
            {currentTier >= 20 && (
