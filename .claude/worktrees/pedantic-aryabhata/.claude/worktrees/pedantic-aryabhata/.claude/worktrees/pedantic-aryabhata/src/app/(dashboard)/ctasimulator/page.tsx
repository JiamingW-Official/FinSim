...
function StatChip({ label, value, sub, color = "text-foreground" }: { label: string; value: string; sub?: string; color?: string }) {
  return (
    <div className="flex flex-col gap-0.5 p-3 rounded-lg bg-muted/60 border border-border">
            ...
            <motion.div key="overview" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                {[
                  ...
                  <Card key={title} className="bg-card/60 border-border">
                    ...
                    <CardContent>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {signals.map((sig) => (
                          <span key={sig} className="px-2 py-0.5 rounded text-xs bg-muted text-muted-foreground border border-border">{sig}</span>
                        ))}
                      </div>
                    </CardContent>
                  </Card>
                      ...
                      <table className="w-full text-xs text-muted-foreground">
                        ...
                        <tbody>
                          {trendSignals.map((sig, i) => (
                            <tr
                              key={i}
                              className={`border-b border-border cursor-pointer transition-colors ${selectedMarket === sig.market ? "bg-muted/30" : "hover:bg-muted/40"}`}
        ...
        <TabsContent value="portfolio" className="data-[state=inactive]:hidden">
          <AnimatePresence mode="wait">
            <motion.div key="portfolio" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
              ...
              {/* Vol target slider */}
              <Card className="mb-4 bg-card/60 border-border">
                <CardHeader className="pb-2">
                  <CardTitle className="flex items-center gap-2 text-sm">
                ...
                <CardContent>
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-3">
                    {[
                      ...
                      <div key={label} className="p-2 rounded bg-muted/60 border border-border">
                ...
                <Card className="bg-card/60 border-border">
                  ...
                  <CardContent>
                    <table className="w-full text-xs text-muted-foreground">
                      ...
                      <tbody>
                        {annualReturns.map((r) => (
                          <tr key={r.year} className="border-b border-border hover:bg-muted/30">
          ...
          <AnimatePresence mode="wait">
            <motion.div key="correlations" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                {corrRegimes.map((r) => (
                  <Card key={r.regime} className="bg-card/60 border-border">
