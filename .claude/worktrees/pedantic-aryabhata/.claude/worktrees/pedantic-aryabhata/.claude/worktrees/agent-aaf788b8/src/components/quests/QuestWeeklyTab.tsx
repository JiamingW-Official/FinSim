  ...
  return (
    <div className="space-y-4">
      ...
      <motion.div
        ...
        className="flex items-center justify-between rounded-lg border border-border/50 bg-muted/10 px-4 py-2.5"
      >
      ...
      {allClaimed && (
        <motion.div
          ...
          className="flex items-center gap-3 rounded-lg border border-green-500/20 bg-green-500/5 px-4 py-3"
        >
          <CheckCircle2 className="h-5 w-5 text-green-400" />
          <div>
            <p className="text-xs font-bold text-green-300">All weekly quests completed!</p>
            <p className="text-[10px] text-green-400/60">New quests arrive Monday.</p>
          </div>
        </motion.div>
      )}
      ...
      <div className="space-y-3">
        {quests.length === 0 ? (
          ...
          >
            ...
            <p className="text-xs text-muted-foreground/60">Weekly quests refresh every Monday morning</p>
          </motion.div>
        ) : (
          quests.map((quest, i) => (
            <motion.div
