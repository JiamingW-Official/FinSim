...
import { GreeksSurface } from "@/components/options/GreeksSurface";
import { UnusualActivityFeed, UnusualActivityStatsBar } from "@/components/options/UnusualActivityFeed";
import { FlowHeatmap } from "@/components/options/FlowHeatmap";
import { PremiumFlow } from "@/components/options/PremiumFlow";
import { StrategyReference } from "@/components/options/StrategyReference";
          ...
          <div className="flex flex-1 flex-col overflow-hidden min-w-0">
            ...
            {!isLoading && !error && (
              <>
                ...
                {/* Strategy Builder tab — with sub-tabs: Builder | Compare */}
                <TabsContent value="strategy" className="mt-0 flex-1 overflow-hidden data-[state=inactive]:hidden">
                  <Tabs defaultValue="builder" className="flex h-full flex-col overflow-hidden">
                    <TabsList className="h-7 w-full shrink-0 justify-start rounded-none border-b border-border bg-card px-2">
                      <TabsTrigger
                        value="builder"
                        className="h-6 rounded-none border-b-2 border-transparent px-3 text-xs data-[state=active]:border-primary data-[state=active]:bg-transparent"
                      >
                    ...
                    <TabsContent value="builder" className="mt-0 flex-1 overflow-hidden data-[state=inactive]:hidden">
                      <StrategyBuilderV2
                        chain={chain}
                        spotPrice={spotPrice}
                        selectedExpiry={selectedExpiry}
                    ...
                    <TabsContent value="compare" className="mt-0 flex-1 overflow-hidden data-[state=inactive]:hidden">
                      <StrategyComparison />
                    </TabsContent>
                  </Tabs>
                </TabsContent>

                {/* Analysis tab — with sub-tabs: Charts | Greeks */}
                <TabsContent value="analysis" className="mt-0 flex-1 overflow-hidden data-[state=inactive]:hidden">
                  <Tabs defaultValue="charts" className="flex h-full flex-col overflow-hidden">
                    <TabsList className="h-7 w-full shrink-0 justify-start rounded-none border-b border-border bg-muted/20 px-2 gap-0">
                      <TabsTrigger
                        value="charts"
                        className="h-6 rounded-none border-b-2 border-transparent px-3 text-xs data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:text-foreground"
                      >
                        Charts
                      </TabsTrigger>
                      <TabsTrigger
                    ...
                    <TabsContent value="charts" className="mt-0 flex-1 overflow-auto data-[state=inactive]:hidden">
                      <AnalysisPanel
                        analytics={analytics}
                        smile={smile}
                    ...
                    <TabsContent value="greeks" className="mt-0 flex-1 overflow-auto data-[state=inactive]:hidden">
                      <GreeksSurface
                        spotPrice={spotPrice}
                        baseIV={analytics.atmIV}
                        dte={chain[0]?.daysToExpiry ?? 30}
                        smile={smile}
                      />
                    </TabsContent>
                  </Tabs>
                ...
                <TabsContent value="unusual" className="mt-0 flex-1 overflow-hidden data-[state=inactive]:hidden">
                  <div className="flex h-full flex-col overflow-hidden">
                    {/* Top: aggregate stats bar (full width) */}
                    <UnusualActivityStatsBar items={unusualActivity} />
                    {/* Middle: premium flow ticker tape (full width, auto-scroll) */}
                    <PremiumFlow items={unusualActivity} />
                    {/* Bottom: 2-col grid — FlowHeatmap (left) + UnusualActivityFeed table (right) */}
                    <div className="flex flex-1 overflow-hidden min-h-0">
                      <div className="w-1/2 shrink-0 overflow-auto border-r border-border/50">
                        <FlowHeatmap chain={chain} spotPrice={spotPrice} />
                      </div>
            ...
            {/* Reference tab — static content, always available */}
            <TabsContent value="reference" className="mt-0 flex-1 overflow-hidden data-[state=inactive]:hidden">
              <StrategyReference />
            </TabsContent>
          </div>
