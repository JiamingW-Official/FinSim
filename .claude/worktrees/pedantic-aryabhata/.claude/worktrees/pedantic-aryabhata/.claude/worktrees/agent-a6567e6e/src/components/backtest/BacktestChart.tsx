    ...
    <div className="relative flex-1 overflow-hidden">
      ...
      {isPreview && bars.length > 0 && !isRunning && (
        <div className="absolute left-3 top-3 z-10 flex items-center gap-2">
          <div className="rounded-lg border border-white/10 bg-zinc-900/90 px-3 py-1.5 text-[11px] text-zinc-400 shadow-lg backdrop-blur">
          ...
          {onRegenerate && (
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-lg border border-white/10 bg-zinc-900/90 text-zinc-400 shadow-lg backdrop-blur transition-colors hover:bg-violet-500/20 hover:text-violet-300 cursor-pointer"
            >
      ...
      {bars.length === 0 && !isRunning && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-muted-foreground">
          <svg className="h-16 w-16 opacity-30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" aria-hidden="true">
            <path d="M3 3v18h18" />
