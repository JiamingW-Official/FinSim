...
export function ChainStatsBar({ analytics, spotPrice }: ChainStatsBarProps) {
  const {
    ...
    expectedMove1SD, heatScore,
  } = analytics;

  const totalVol = totalCallVolume + totalPutVolume;
  const totalOI = totalCallOI + totalPutOI;

  const ivVsHvClass = ivVsHvSpread > 0.02 ? "text-orange-400" : "text-green-400";
  ...
  const ivRankClass =
    ivRank >= 60 ? "text-red-400" : ivRank >= 30 ? "text-amber-400" : "text-green-400";

  const heatScoreClass =
    heatScore >= 70 ? "text-red-400" : heatScore >= 40 ? "text-amber-400" : "text-green-400";
  ...
  const chips: StatChip[] = [
    ...
    {
      label: "Heat Score",
      value: heatScore.toFixed(0),
      colorClass: heatScoreClass,
    },
  ];
