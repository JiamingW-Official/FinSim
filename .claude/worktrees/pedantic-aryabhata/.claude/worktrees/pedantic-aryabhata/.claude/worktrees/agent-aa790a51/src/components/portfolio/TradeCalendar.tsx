...
function getColor(pnl: number, maxAbs: number): string {
  ...
  if (pnl > 0) {
    if (ratio > 0.6) return "bg-green-500/80";
    if (ratio > 0.3) return "bg-green-500/50";
...
export function TradeCalendar() {
  ...
  const sellCount = useMemo(() => tradeHistory.filter((t) => t.side === "sell").length, [tradeHistory]);
  if (sellCount === 0) {
    return (
    ...
    <div>
      ...
      <div className="mb-2 h-4 text-[10px] text-muted-foreground tabular-nums">
        {hovered ? (
          <span>
            {hovered.date}{" "}
            <span className={cn("font-bold", hovered.pnl >= 0 ? "text-green-400" : "text-red-400")}>
      ...
      <div className="mt-2 flex items-center justify-end gap-1.5 text-[8px] text-muted-foreground/50">
        ...
        <div className="flex gap-[2px]">
          ...
          <div className="h-[10px] w-[10px] rounded-[2px] bg-green-500/80" />
        </div>
        <span>Profit</span>
      </div>
    </div>
