    ...
    <div className="flex h-full flex-col items-center justify-center gap-5 p-6 overflow-y-auto">
      ...
      >
        <StatRow
          ...
          valueColor={result.playerPnLPercent >= 0 ? "text-emerald-400" : "text-red-400"}
          opponent={`${result.opponentPnLPercent >= 0 ? "+" : ""}${result.opponentPnLPercent.toFixed(2)}%`}
          opponentColor={result.opponentPnLPercent >= 0 ? "text-emerald-400/60" : "text-red-400/60"}
        />
        <StatRow
      ...
      >
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="flex items-center gap-1.5 rounded-lg bg-red-500 px-6 py-2.5 text-sm font-semibold text-foreground transition-colors hover:bg-red-400"
        >
        ...
        <motion.button
          ...
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          className="rounded-lg border border-border bg-muted/20 px-6 py-2.5 text-sm font-semibold text-muted-foreground transition-colors hover:bg-muted/50"
        >
          Back to Lobby
        </motion.button>
      </motion.div>
