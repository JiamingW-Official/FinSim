...
export function ScenarioCard({
  ...
  return (
    ...
    >
      ...
      {isLocked && (
        <motion.div
          className="absolute inset-0 flex items-center justify-center rounded-lg bg-background/70 backdrop-blur-[3px] z-10"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="shrink-0 flex flex-col items-center gap-1">
          {bestResult ? (
            <motion.div
              className={cn(
                "flex h-11 w-11 items-center justify-center rounded-lg border-2 text-xl font-black",
                gradeConfig?.border,
                gradeConfig?.bg,
                gradeConfig?.text,
