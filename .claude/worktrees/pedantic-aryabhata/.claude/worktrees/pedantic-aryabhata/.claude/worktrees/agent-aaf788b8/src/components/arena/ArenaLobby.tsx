  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div
        ...
        className="rounded-lg border border-border/50 bg-gradient-to-r from-red-500/5 via-transparent to-red-500/5 p-4"
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-black tabular-nums text-foreground">{elo}</span>
              <span className="text-xs text-muted-foreground">ELO</span>
            </div>
            {totalMatches === 0 && (
              <p className="text-[10px] text-muted-foreground/60 mt-0.5">Play your first match to start ranking!</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-4 text-xs">
            ...
            {currentStreak > 0 && (
              ...
              >
                ...
                <div className="text-muted-foreground/60">Streak</div>
              </motion.div>
            )}
            {bestStreak > 0 && currentStreak === 0 && (
              <div className="text-center">
                <div className="font-bold tabular-nums text-muted-foreground">{bestStreak}</div>
                <div className="text-muted-foreground/60">Best</div>
              </div>
            )}
          </div>
        </div>
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-muted-foreground/60">{config.timeLimitSeconds}s</span>
                </div>
              </div>
              ...
              <div className="space-y-1">
                {Object.entries(config.scoringWeights).map(([key, weight]) => (
                  <div key={key} className="flex items-center gap-2">
                    <span className="text-[9px] w-12 text-muted-foreground/60 capitalize">{key === "riskControl" ? "Risk" : key}</span>
                    <div className="flex-1 h-1 rounded-full bg-muted/30">
                      <motion.div
                        className={cn("h-full rounded-full", WEIGHT_COLORS[key] ?? "bg-muted/80")}
                        initial={{ width: 0 }}
      ...
      <div className="space-y-2">
        {recent.map((m, i) => (
          <motion.div
            ...
            className="flex items-center gap-3 rounded-lg border border-border/20 bg-muted/5 px-3 py-1.5 text-xs"
          >
            <span className={cn(
              "flex h-5 w-5 items-center justify-center rounded text-[10px] font-black",
              m.playerWon ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400",
            )}>
              {m.playerWon ? "W" : "L"}
            </span>
            <span className="text-muted-foreground flex-1 truncate">vs {m.opponentName}</span>
            <span className="text-muted-foreground/60 tabular-nums">{m.playerScore} - {m.opponentScore}</span>
            <span className={cn("tabular-nums font-bold", m.eloChange >= 0 ? "text-green-400" : "text-red-400")}>
