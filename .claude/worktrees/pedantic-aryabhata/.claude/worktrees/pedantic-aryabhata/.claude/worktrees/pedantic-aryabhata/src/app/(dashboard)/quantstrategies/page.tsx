        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {selected.factors.map((f) => (
              <Badge key={f} variant="outline" className="border-border text-muted-foreground text-xs">
                {f}
              </Badge>
            ))}
          </div>
            ...
            <table className="w-full text-xs text-muted-foreground">
              ...
              <tbody>
                {FACTOR_MODELS.map((m) => (
                  <tr
                    key={m.shortName}
                    className={`border-b border-border cursor-pointer hover:bg-card/80/50 transition-colors ${
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {MOMENTUM_STRATEGIES.map((s, i) => (
          <button
            ...
            className={`text-left rounded-md border p-3 transition-all ${
              selected === i ? "bg-card/80" : "border-border bg-card hover:border-border"
            }`}
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-muted-foreground">
                ...
                <tbody>
                  {ML_FEATURES.map((f) => (
                    <tr key={f.name} className="border-b border-border hover:bg-card/80/40 transition-colors">
