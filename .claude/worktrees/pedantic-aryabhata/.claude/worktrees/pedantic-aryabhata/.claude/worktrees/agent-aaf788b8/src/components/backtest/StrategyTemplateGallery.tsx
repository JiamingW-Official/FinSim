...
const DIFFICULTY_COLORS = ["", "text-green-400", "text-blue-400", "text-amber-400", "text-orange-400", "text-rose-400"];
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative max-h-[85vh] w-full max-w-5xl overflow-hidden rounded-lg border border-border bg-card"
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-border/50 px-4 sm:px-6 py-4">
          ...
          <button onClick={onClose} className="rounded-lg p-2 text-muted-foreground transition-colors hover:bg-muted/30 hover:text-foreground/80">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex h-[calc(85vh-64px)] overflow-hidden">
          {/* Left: Card Grid */}
          <div className="w-[400px] space-y-2 overflow-y-auto border-r border-border/50 p-4">
            {STRATEGY_TEMPLATES.map((template) => {
              ...
              return (
                ...
                >
                  <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${catInfo?.bgColor ?? "bg-muted/50"}`}>
                    <Icon className={`h-5 w-5 ${catInfo?.color ?? "text-muted-foreground"}`} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-foreground">{template.name}</span>
                      <ChevronRight className={`h-3 w-3 text-muted-foreground/60 transition-transform ${isSelected ? "rotate-90" : ""}`} />
                    </div>
                    ...
                    <div className="mt-1.5 flex items-center gap-2">
                      ...
                      <span className="flex items-center gap-0.5">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star
                            key={i}
                            className={`h-2.5 w-2.5 ${i < template.difficulty ? DIFFICULTY_COLORS[template.difficulty] : "text-muted-foreground/40"}`}
            ...
            <AnimatePresence mode="wait">
              {selected ? (
                ...
                >
                  ...
                  {/* Signals */}
                  <Section icon={<Crosshair className="h-4 w-4 text-green-400" />} title="Trading Signals">
                      ...
                      {selected.keyParameters.map((param, i) => (
                        <div key={i} className="rounded-lg border border-border/50 bg-muted/30 px-3 py-2">
                          <div className="flex items-center justify-between">
                            <span className="text-[11px] font-medium text-foreground/80">{param.name}</span>
                            <span className="rounded bg-violet-500/20 px-1.5 py-0.5 text-[10px] font-mono font-semibold text-violet-300">
                              {param.default}
                            </span>
                          </div>
                          <div className="mt-0.5 text-[10px] text-muted-foreground/60">{param.description}</div>
                        </div>
                      ))}
                    </div>
                  </Section>
                  ...
                  <button
                    ...
                    className="w-full rounded-lg bg-gradient-to-r from-violet-600 to-fuchsia-600 py-3 text-sm font-bold text-white shadow-sm shadow-violet-500/20 transition-all hover:shadow-violet-500/40"
                  >
                    Use This Strategy
                  </button>
                </motion.div>
                ...
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="flex h-full flex-col items-center justify-center gap-3 text-muted-foreground/60"
                >
