...
export default function AnalyticsPanel({ result }: AnalyticsPanelProps) {
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      <div>
        <h3 className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Risk-Adjusted Metrics</h3>
      ...
      {/* Drawdown Chart */}
      <div>
        <h3 className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Drawdown</h3>
        <DrawdownChart
          data={drawdownCurve}
      ...
      <div>
        <h3 className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Period Returns (20-bar periods)</h3>
        <div className="flex flex-wrap gap-1">
          {periodReturns.map((p) => {
            ...
            return (
              <div
                key={p.period}
                className="group relative flex h-10 w-10 flex-col items-center justify-center rounded-md border border-border/50 text-[9px]"
                style={{
              ...
              >
                ...
                {/* Tooltip */}
                <div className="absolute -top-8 left-1/2 z-10 hidden -translate-x-1/2 rounded bg-muted/50 px-2 py-1 text-[10px] text-foreground/80 shadow-sm group-hover:block whitespace-nowrap">
                  {p.trades} trades
                </div>
              </div>
            );
      ...
      <div>
        <h3 className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Trade P&L Distribution</h3>
        <TradeDistribution trades={trades} />
      </div>

      {/* MAE/MFE scatter summary */}
      <div>
        <h3 className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Trade Quality (MAE / MFE)</h3>
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-lg border border-border/50 bg-muted/30 p-3">
            <div className="text-[10px] text-muted-foreground">Avg Max Adverse</div>
            <div className="text-sm font-bold text-rose-400">-{metrics.avgMAE.toFixed(2)}%</div>
            <div className="text-[9px] text-muted-foreground/60">Avg worst drawdown per trade</div>
          </div>
          <div className="rounded-lg border border-border/50 bg-muted/30 p-3">
            <div className="text-[10px] text-muted-foreground">Avg Max Favorable</div>
            <div className="text-sm font-bold text-green-400">+{metrics.avgMFE.toFixed(2)}%</div>
            <div className="text-[9px] text-muted-foreground/60">Avg best unrealized gain per trade</div>
          </div>
        </div>
      </div>
    </div>
...
function MiniMetric({ label, value, good }: { label: string; value: string | number; good?: boolean }) {
  return (
    <div className="rounded-lg border border-border/50 bg-muted/30 px-2 py-1.5">
      ...
      <div className={`text-sm font-bold ${good === undefined ? "text-foreground" : good ? "text-green-400" : "text-rose-400"}`}>
...
function TradeDistribution({ trades }: { trades: { pnl: number }[] }) {
  if (trades.length === 0) return <div className="py-2 text-center text-xs text-muted-foreground">No trades to display in distribution</div>;
    ...
    <div className="flex h-16 items-end gap-px">
      {bins.map((count, i) => {
        ...
        return (
          ...
          >
            {count > 0 && (
              <div className="absolute -top-6 left-1/2 z-10 hidden -translate-x-1/2 rounded bg-muted/50 px-1.5 py-0.5 text-[9px] text-foreground/80 shadow group-hover:block whitespace-nowrap">
