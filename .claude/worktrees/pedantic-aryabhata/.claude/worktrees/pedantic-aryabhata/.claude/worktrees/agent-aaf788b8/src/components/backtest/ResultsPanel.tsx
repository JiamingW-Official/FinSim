...
const GRADE_CONFIG = {
  ...
  C: { color: "text-muted-foreground", bg: "bg-muted/30", border: "border-border", glow: "", icon: Dumbbell, label: "Keep trying!" },
} as const;
...
export default function ResultsPanel({ result, monteCarloResult, xpEarned, onSave, onRerun }: ResultsPanelProps) {
  ...
  return (
    <div className="flex h-full w-[360px] flex-col border-l border-border/50 bg-muted/10">
      {/* Tab Bar */}
      ...
      <div className="flex-1 overflow-y-auto">
        {tab === "overview" && (
          <div className="space-y-3 p-4">
            ...
            <motion.div
              ...
              className={`grade-reveal flex flex-col items-center gap-2 rounded-lg border p-4 ${config.bg} ${config.border} ${config.glow}`}
            >
            ...
            <div className="space-y-1">
              <h3 className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Equity Curve</h3>
        ...
        {tab === "trades" && (
          <div className="space-y-1 p-4">
            ...
            <div className="space-y-1">
              {trades.map((t) => (
                <div key={t.id} className="rounded-lg border border-border/50 bg-muted/30 px-3 py-2 text-xs">
                  <div className="flex items-center justify-between">
                    <span className="text-foreground/80">Bar {t.entryBar} → {t.exitBar}</span>
                  ...
                  <div className="mt-1 flex items-center gap-3 text-[10px]">
                    ...
                    <span className="text-muted-foreground/60">Fee ${t.commission.toFixed(2)}</span>
                  </div>
                </div>
              ))}
              {trades.length === 0 && (
      ...
      <div className="flex gap-2 border-t border-border/50 p-3">
        ...
        <button onClick={onRerun} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-border bg-muted/30 py-2 text-xs font-medium text-foreground/80 transition-colors hover:bg-muted/50">
...
function MetricCard({ icon, label, value, sub, positive }: { icon: React.ReactNode; label: string; value: string; sub?: string; positive?: boolean }) {
  return (
    <div className="rounded-lg border border-border/50 bg-muted/30 px-3 py-2">
      ...
      <div className={`mt-0.5 text-sm font-bold ${positive === undefined ? "text-foreground" : positive ? "text-green-400" : "text-rose-400"}`}>
        {value}
      </div>
