...
export default function StrategyPanel({
  ...
  return (
    <div className="flex h-full w-[300px] flex-col gap-2.5 overflow-y-auto border-r border-border bg-card/50 p-3">
      {/* Header */}
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 border border-border px-3 py-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground"
      >
      ...
      {savedStrategies.length > 0 && (
        <div>
          <button
            ...
            className="flex w-full items-center justify-between border border-border bg-muted/20 px-3 py-1.5 text-xs text-muted-foreground transition-colors hover:bg-muted/50"
          >
              ...
              >
                {savedStrategies.map((s) => (
                  <button
                    ...
                    className="w-full border border-border bg-muted/20 px-3 py-1.5 text-left text-xs transition-colors hover:bg-muted/50"
                  >
      ...
      <Section label="Ticker">
        <select
          ...
          className="w-full border border-border bg-muted/20 px-3 py-1.5 text-xs text-foreground outline-none focus:border-border"
        >
      ...
      <Section label={`Entry Rules (${entryRules.length}/3)`} icon={<Crosshair className="h-3 w-3 text-emerald-400" />}>
        <div className="space-y-1.5">
          {entryRules.map((rule) => (
            <div
              key={rule.id}
              className="flex items-center justify-between border border-border bg-muted/30 px-3 py-1.5"
            >
          ...
          {entryRules.length < 3 && (
            <button
              ...
              className="flex w-full items-center justify-center gap-1 border border-dashed border-border py-2 text-xs text-muted-foreground transition-colors hover:border-border hover:text-foreground"
            >
        ...
        <AnimatePresence>
          {showRulePicker && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="mt-2 max-h-52 space-y-1 overflow-y-auto border border-border bg-card/95 p-2"
            >
        ...
        <div className="space-y-2">
          ...
          {positionSizing.kind === "fixed_shares" && (
            <input
              ...
              className="w-full border border-border bg-muted/20 px-3 py-1.5 text-xs font-mono text-foreground outline-none focus:border-border"
            />
          )}
