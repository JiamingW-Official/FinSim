...
export function MiniTradePanel({
  ...
  return (
    <div className="glass-subtle rounded-md border border-border p-3 space-y-2.5">
      {/* Price + Cash */}
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-1.5">
            <motion.button type="button" onClick={handleBuy}
              ...
              whileTap={{ scale: 0.95 }}
              className="flex-1 flex items-center justify-center gap-1 rounded-lg bg-emerald-500 py-2 text-xs font-semibold text-foreground transition-colors hover:brightness-110 disabled:opacity-30">
            ...
            <motion.button type="button" onClick={handleShort}
              ...
              whileTap={{ scale: 0.95 }}
              className="flex-1 flex items-center justify-center gap-1 rounded-lg bg-red-500 py-2 text-xs font-semibold text-foreground transition-colors hover:brightness-110 disabled:opacity-30">
            ...
            {position && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
                <motion.button type="button" onClick={handleClose}
                  ...
                  className="w-full flex items-center justify-center gap-1.5 rounded-lg border border-border bg-muted/20 py-1.5 text-[11px] font-semibold text-muted-foreground transition-colors hover:bg-muted/50 disabled:opacity-30">
                  <X className="h-3 w-3" />
        ...
        <div className="flex items-center gap-2">
          {showLegacyBuy && (
            <>
              ...
              <motion.button type="button" onClick={handleBuy}
                ...
                whileTap={{ scale: 0.95 }}
                className="flex-1 rounded-lg bg-emerald-500 py-2 text-xs font-semibold text-foreground transition-colors hover:brightness-110 disabled:opacity-40">
                Buy {qty}
              </motion.button>
            </>
          )}
          {showLegacySell && position && (
            <motion.button type="button" onClick={handleSell}
              disabled={allComplete || atEnd} whileTap={{ scale: 0.95 }}
              className="flex-1 rounded-lg bg-red-500 py-2 text-xs font-semibold text-foreground transition-colors hover:brightness-110 disabled:opacity-40">
