...
function RankMedal({ rank }: { rank: number }) {
  if (rank === 1) {
    return (
      <span className="text-base medal-shimmer">🥇</span>
    );
  }
...
export function RankRow({ entry, dimConfig, index, onSelect, isSelected }: RankRowProps) {
  ...
  const isClickable = !entry.isUser && onSelect !== undefined;
  ...
  return (
    ...
    >
      ...
      >
        {entry.isUser ? "🫵" : getInitials(entry.name)}
      </div>
          ...
          className={cn(
            ...
            dimConfig.id === "total_pnl"
              ...
              : dimConfig.id === "risk_control"
                ? value <= 15 ? "text-green-400" : value <= 30 ? "text-amber-400" : "text-red-400"
                : "text-foreground",
          )}
        >
          {formatted}
