...
function StatChip({
 ...
 purple: "bg-muted/10 border-border text-foreground",
 ...
 <div className="text-xs text-muted-foreground opacity-70">{label}</div>
...
function IPOTimelineTab() {
 ...
 <h3 className="text-sm font-semibold text-foreground/80 mb-4">IPO Process — Click Each Step for Detail</h3>
 ...
 >
 <span className="w-4 h-4 rounded-full flex items-center justify-center text-xs text-muted-foreground font-semibold"
 ...
 <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs text-muted-foreground font-medium"
 style={{ backgroundColor: selectedStep.color + "30", color: selectedStep.color }}>
 {selectedStep.id}
 </span>
 <h4 className="text-base font-semibold text-foreground">{selectedStep.name}</h4>
 </div>
 <div className="flex items-center gap-2 mt-1">
 <span className="text-xs text-foreground/50">{selectedStep.duration}</span>
 </div>
 </div>
 <button onClick={() => setSelectedStep(null)} className="text-foreground/30 hover:text-foreground/70">
 <X className="w-4 h-4" />
 </button>
 </div>

 <p className="text-sm text-foreground/70 leading-relaxed">{selectedStep.description}</p>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div>
 <div className="text-xs font-semibold text-foreground/50 mb-2">Key Activities</div>
...
function BookBuildingTab() {
 ...
 {[
 { id: "bookbuild" as const, label: "Book Building" },
 { id: "greenshoe" as const, label: "Greenshoe Option" },
 { id: "pricing" as const, label: "IPO Pricing" },
 { id: "underwriter" as const, label: "Underwriter Economics" },
 ].map(({ id, label }) => (
 <button
 key={id}
 onClick={() => setActiveSection(id)}
 className={cn(
 "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-muted-foreground font-medium border transition-colors",
 activeSection === id
 ? "bg-muted/10 border-primary/50 text-foreground"
 : "bg-foreground/[0.03] border-border text-foreground/50 hover:text-foreground/80"
 )}
 >
 {label}
 </button>
 ...
 <div className="w-5 h-5 rounded-full bg-muted/10 text-foreground text-xs flex items-center justify-center flex-shrink-0 mt-0.5">
 {i + 1}
 </div>
 <div>
 ...
 <div className="text-xs font-medium text-foreground mb-2">Institutional Advantages</div>
 <ul className="space-y-1 text-xs text-foreground/60">
 <li className="flex items-start gap-1.5">Priority allocation in hot deals</li>
 <li className="flex items-start gap-1.5">Can indicate at IPO price (firm commitment)</li>
 <li className="flex items-start gap-1.5">Access to management roadshow 1-on-1s</li>
 <li className="flex items-start gap-1.5">Receive analyst research before quiet period ends</li>
 </ul>
 </div>
 <div className="bg-muted/5 border border-border rounded-lg p-4">
 <div className="text-xs font-medium text-foreground mb-2">Retail Options</div>
 <ul className="space-y-1 text-xs text-foreground/60">
 <li className="flex items-start gap-1.5">Directed Share Programs (employees, customers)</li>
 <li className="flex items-start gap-1.5">Broker IPO allocations (Schwab, Fidelity, TD)</li>
 <li className="flex items-start gap-1.5">Retail platforms: IBKR, Robinhood (select deals)</li>
 <li className="flex items-start gap-1.5">Reddit-style allocation (RDDT precedent)</li>
 </ul>
 </div>
 </div>
 </div>
 ...
 <div className="mt-3 text-xs text-foreground/50 leading-relaxed">
 <strong className="text-foreground/70">Why underpricing persists:</strong> Underwriters maintain investor relationships, ensure aftermarket stability, reduce lawsuit risk from price drops, and signal deal quality. The 7% gross spread is direct compensation; underpricing is indirect.
 </div>
 </div>
 </div>
 )}
 ...
 <div className="text-xs text-foreground/45 italic">{m.note}</div>
 </div>
 ))}
 </div>
 </div>
...
function MarketAnalysisTab() {
 ...
 <th className="px-3 py-2.5 text-right cursor-pointer text-foreground/50 hover:text-foreground/80 select-none"
 onClick={() => toggleSort("offerPrice")}>
 Offer {sortCol === "offerPrice" && (sortDir === -1 ? "↓" : "↑")}
 </th>
 <th className="px-3 py-2.5 text-right text-foreground/50">1st Day</th>
 <th className="px-3 py-2.5 text-right cursor-pointer text-foreground/50 hover:text-foreground/80 select-none"
 ...
 <th className="px-4 py-2.5 text-left text-foreground/50">Notes</th>
 </tr>
 </thead>
 <tbody>
 {sorted.map((ipo) => {
 ...
 <td className="px-3 py-2.5 text-right text-foreground/60">{ipo.year}</td>
 <td className="px-3 py-2.5 text-right text-foreground/85">
 ...
 <td className="px-4 py-2.5 text-foreground/45 max-w-[220px]">
 ...
 <div className="mt-2 text-xs text-foreground/45 leading-relaxed">
 ...
 <div className="w-28 text-xs text-foreground/60 flex-shrink-0">{row.sector}</div>
 <div className="flex-1 flex items-center gap-2">
 <div className="w-12 text-xs text-foreground/40 text-right">2021:</div>
 <div className="flex-1 bg-foreground/5 rounded-full h-2">
 <div className="h-2 rounded-full bg-muted/50" style={{ width: `${row.pct2021}%` }} />
 </div>
 <div className="w-8 text-xs text-foreground/60">{row.pct2021}%</div>
 </div>
 <div className="flex-1 flex items-center gap-2">
 <div className="w-12 text-xs text-foreground/40 text-right">2023:</div>
 <div className="flex-1 bg-foreground/5 rounded-full h-2">
 ...
 <div className="w-8 text-xs text-foreground/60">{row.pct2023}%</div>
 </div>
 </div>
 ))}
 </div>
 ...
 <div key={r.rule} className={cn("rounded-lg p-3 border text-xs text-muted-foreground", r.type === "post" ? "bg-muted/5 border-border" : "bg-amber-500/5 border-amber-500/15")}>
 <div className={cn("font-medium mb-1", r.type === "post" ? "text-foreground" : "text-amber-400")}>{r.rule}</div>
...
function AlternativesTab() {
 ...
 <th className="px-3 py-2.5 text-left text-foreground">SPAC</th>
 </tr>
 </thead>
 <tbody>
 ...
 {dim.advantage === "traditional" && <CheckCircle2 className="w-3 h-3 text-foreground inline mr-1" />}
 {dim.traditional}
 </td>
 <td className={cn("px-3 py-2.5 text-foreground/65", dim.advantage === "direct" && advColors.direct)}>
 {dim.advantage === "direct" && <CheckCircle2 className="w-3 h-3 text-green-400 inline mr-1" />}
 {dim.directListing}
 </td>
 <td className={cn("px-3 py-2.5 text-foreground/65", dim.advantage === "spac" && advColors.spac)}>
 {dim.advantage === "spac" && <CheckCircle2 className="w-3 h-3 text-foreground inline mr-1" />}
 {dim.spac}
 </td>
 </tr>
 );
 ...
 <span className="text-foreground/50">Ref: <span className="text-foreground/80">${dl.ref}</span></span>
 <span className="text-foreground/50">Open: <span className="text-green-400">${dl.open}</span></span>
 </div>
 <p className="text-xs text-foreground/50">{dl.note}</p>
 </div>
 ))}
 </div>
 </div>
 ...
 <div key={i} className="flex gap-3">
 <span className="w-5 h-5 rounded-full bg-muted/10 text-foreground text-xs flex items-center justify-center flex-shrink-0 mt-0.5 font-medium">{i + 1}</span>
 <div>
 <div className="text-xs font-medium text-foreground/80">{s.step}</div>
 <p className="text-xs text-foreground/55 leading-relaxed">{s.desc}</p>
 </div>
 ...
 <div className="flex gap-2"><span className="text-foreground/40">Expiry:</span>5 years from SPAC IPO date</div>
 <div className="flex gap-2"><span className="text-foreground/40">Callable:</span>If stock trades above $18 for 20 of 30 days</div>
 <div className="flex gap-2"><span className="text-foreground/40">Dilution:</span>Warrants can add 15–20% dilution to existing shareholders</div>
 </div>
 </div>
 </div>
 </div>
...
export default function IPOMechanicsPage() {
 ...
 <TabsTrigger value="alternatives" className="rounded-none border-b-2 border-transparent data-[state=active]:border-foreground data-[state=active]:bg-transparent data-[state=active]:shadow-none px-3 py-2 text-xs text-muted-foreground data-[state=active]:text-foreground">
 Alternatives
 </TabsTrigger>
 </TabsList>
