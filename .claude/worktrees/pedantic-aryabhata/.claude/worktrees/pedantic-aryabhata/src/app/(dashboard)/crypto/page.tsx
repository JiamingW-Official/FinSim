...
function FearGreedGauge({ value }: { value: number }) {
 ...
 <div className="text-2xl font-semibold font-mono tabular-nums" style={{ color: needleColor }}>{value}</div>
...
function MarketsTab({ rows, seed }: { rows: CryptoRow[]; seed: number; }) {
 ...
 <tbody>
 {rows.map((r, i) => (
 <tr key={r.symbol} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
 <td className="py-2.5 px-3 text-muted-foreground tabular-nums text-xs">{i + 1}</td>
 <td className="py-2.5 px-3">
 <div className="flex items-center gap-2">
 <span className="font-medium text-xs text-foreground w-12">{r.symbol}</span>
 <span className="text-xs text-muted-foreground hidden sm:inline">{r.name}</span>
 </div>
 </td>
 <td className="py-2.5 px-3 text-right tabular-nums font-medium text-xs text-muted-foreground">{fmtPrice(r.price)}</td>
 ...
 <tr key={m.symbol} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
 <td className="py-2.5 px-3 font-medium text-foreground">{m.symbol}</td>
...
function TradeTab({ rows, seed }: { rows: CryptoRow[]; seed: number }) {
 ...
 className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm tabular-nums focus:outline-none focus:ring-1 focus:ring-primary"
 />
 </div>
 ...
 return (
 <div key={pos.symbol} className="rounded-md border border-border p-2.5 text-xs text-muted-foreground">
...
function YieldFarmingSimulator() {
 ...
 {/* Selected farm info */}
 <div className="rounded-md bg-muted/20 border border-border px-3 py-2.5 text-xs text-muted-foreground flex items-center justify-between">
 <div>
 ...
 <div className="rounded-md bg-muted/30 border border-border px-3 py-2 text-xs text-muted-foreground leading-relaxed">
 ...
 {/* Compound effect */}
 {deposit > 0 && (
 <div className="rounded-md border border-border p-3 text-xs text-muted-foreground flex flex-col gap-2">
...
function DeFiTab({ seed }: { seed: number }) {
 ...
 selectedPool?.id === pool.id ? "bg-muted/30" : "hover:bg-muted/20",
 )}
 >
 <td className="py-2.5 px-3">
 <div className="font-medium text-xs text-muted-foreground">{pool.pair}</div>
 ...
 className="text-xs rounded-md border border-border px-2.5 py-1 text-muted-foreground hover:text-foreground hover:bg-muted/20 transition-colors"
 >
 Add
 </button>
 </td>
 ...
 <div key={i} className="rounded-md border border-border p-2.5 text-xs text-muted-foreground grid grid-cols-2 gap-x-4 gap-y-1">
 <div className="col-span-2 font-medium text-foreground">{pos.pair}</div>
 ...
 <div className="flex items-center justify-between rounded-md bg-muted/30 px-3 py-2.5 text-xs text-muted-foreground">
...
function OnChainTab({ seed }: { seed: number }) {
 ...
 <div key={tier.label} className="rounded-md border border-border p-3 text-center">
 ...
 <div key={s.label} className="rounded-md border border-border p-2.5 text-xs text-muted-foreground">
 ...
 <tbody>
 {whales.map((w) => (
 <tr key={w.hash} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
 <td className="py-2.5 px-3 font-mono text-muted-foreground">{w.wallet}</td>
 <td className="py-2.5 px-3 text-right tabular-nums font-medium">
 ${(w.amount / 1_000_000).toFixed(2)}M
 </td>
 <td className="py-2.5 px-3 font-medium text-foreground">{w.asset}</td>
...
function EducationTab() {
 ...
 <div className="flex flex-col gap-2">
 {DEFI_GLOSSARY.map((g) => (
 <div key={g.term} className="rounded-lg border border-border bg-card overflow-hidden">
 <button
 type="button"
...
function NftTab({ seed }: { seed: number }) {
 ...
 <tbody>
 {collections.map((c) => (
 <tr key={c.symbol} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
 <td className="py-2.5 px-3">
 ...
 <div className="flex flex-col gap-2">
 {COMPARISON.map((a) => (
 <div key={a.asset} className="rounded-md border border-border p-3 text-xs text-muted-foreground">
 ...
 <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-muted text-xs font-medium text-foreground">
 {i + 1}
 </div>
 <div>
...
function CryptoPortfolioTab({ rows }: { rows: CryptoRow[] }) {
 ...
 return (
 <div key={a.symbol} className="rounded-md border border-border p-2.5 text-xs text-muted-foreground flex items-start gap-2">
 ...
 className="w-28 rounded-md border border-border bg-background px-3 py-2 text-sm tabular-nums focus:outline-none focus:ring-1 focus:ring-primary"
 />
 <button
 type="button"
 onClick={addAsset}
 ...
 <span className="font-medium text-foreground w-10">{s.symbol}</span>
 <span className="text-muted-foreground">
 is {s.diff > 0 ? "over" : "under"}-weight by{" "}
 <span className={cn("font-medium", s.diff > 0 ? "text-amber-500" : "text-foreground")}>
 ...
 return (
 <div key={a.symbol} className="flex items-center gap-2 text-xs text-muted-foreground">
 <span className="font-medium text-foreground w-10">{a.symbol}</span>
 ...
 <div className="rounded-md bg-muted/10 border border-border px-3 py-2 text-xs text-muted-foreground leading-relaxed">
