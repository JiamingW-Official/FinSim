...
function HeadlineStory({ headline, onClose }: { headline: NewsHeadline; onClose: () => void }) {
 ...
 exit={{ opacity: 0, y: -8 }}
 className="border border-border rounded-lg bg-card p-4 relative"
 >
 <button
 onClick={onClose}
...
function ArticleCard({ article }: { article: Article }) {
 ...
 <span key={t} className="text-xs font-mono bg-muted/50 text-foreground px-1.5 py-0.5 rounded border border-border cursor-pointer hover:bg-muted transition-colors">
 ${t}
 </span>
 ))}
 </div>
...
function EarningsCalendar({ entries }: { entries: EarningsEntry[] }) {
 ...
 className="w-full text-left border border-border rounded-lg bg-card p-3 hover:bg-muted/30 transition-colors"
 >
 ...
 className="overflow-hidden"
 >
 <div className="border border-t-0 border-border rounded-b-lg bg-muted/20 p-3">
...
function AnalystRatings({ ratings }: { ratings: AnalystRating[] }) {
 ...
 className="text-xs bg-card border border-border rounded px-2 h-6 text-foreground cursor-pointer"
 >
