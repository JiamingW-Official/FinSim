...
function BusinessCycleClock() {
 ...
 {CYCLE_PHASES.map((p, i) => (
 <div key={p.label} className={cn("rounded border border-border p-2", i === CURRENT_PHASE_IDX && "border-border")}>
...
function CyclePerformanceMatrix() {
 ...
 <tbody>
 {SECTORS_11.map((s) => (
 <tr key={s.name} className="border-b border-border last:border-0 hover:bg-muted/10">
 ...
 <div className="flex gap-3 px-4 py-2 text-xs text-muted-foreground border-t border-border">
...
function RSRankingTable({ ranked }: { ranked: SectorRS[] }) {
 ...
 <div className="flex gap-4 px-4 py-2 text-xs text-muted-foreground border-t border-border">
...
function FactorCycleGrid({ factors }: { factors: FactorRS[] }) {
 ...
 const isActive = i === CURRENT_PHASE_IDX;
 return (
 <div key={p.label} className={cn("rounded border p-3", isActive ? "border-border" : "border-border")}>
...
function RegionalSectorLeadership({ regionals }: { regionals: RegionalData[] }) {
 ...
 {regionals.map((r) => (
 <div key={r.region} className="rounded border border-border bg-muted/10 p-3">
...
export default function SectorRotationStrategy() {
 ...
 <div className="rounded border border-border bg-muted/10 p-3 space-y-1.5">
