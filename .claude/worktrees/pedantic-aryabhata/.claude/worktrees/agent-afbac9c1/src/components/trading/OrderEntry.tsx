...
import { QuickSizeBar } from "./QuickSizeBar";
...
import { CounterpartyMatchCard } from "./CounterpartyPanel";
...
export function OrderEntry() {
  // Refs to hold latest price/portfolioValue so position-size handler is stable
  const priceRef = useRef(0);
  const portfolioValueRef = useRef(0);
  ...
  // Keyboard shortcuts: B = buy side, S = sell side, position-size = set qty by portfolio %
  useEffect(() => {
    const onBuy = () => setSide("buy");
    const onSell = () => setSide("sell");
    const onPositionSize = (e: Event) => {
      const { pct } = (e as CustomEvent<{ pct: number }>).detail;
      const p = priceRef.current;
      const pv = portfolioValueRef.current;
      if (!p || p <= 0 || !pv || pv <= 0) return;
      const dollarAmount = pv * (pct / 100);
      const shares = Math.max(1, Math.floor(dollarAmount / p));
      setQuantity(String(shares));
    };
    window.addEventListener("finsim:focus-buy", onBuy);
    window.addEventListener("finsim:focus-sell", onSell);
    window.addEventListener("finsim:position-size", onPositionSize);
    return () => {
      window.removeEventListener("finsim:focus-buy", onBuy);
      window.removeEventListener("finsim:focus-sell", onSell);
      window.removeEventListener("finsim:position-size", onPositionSize);
    };
  }, []);
  ...
  // Sync refs so the position-size event handler always reads fresh values
    ...
    >
      ...
      <div className="flex items-baseline justify-between">
        ...
        <span
          role="status"
          aria-live="polite"
          aria-label={`${currentTicker} current price: ${price > 0 ? formatCurrency(animatedPrice) : "unavailable"}`}
          className={cn(
      ...
      <div role="group" aria-label="Quick quantity presets" className="flex gap-1">
        {[1, 5, 10, 25, 50, 100].map((q) => (
          <button
            key={q}
            type="button"
            aria-label={`Set quantity to ${q} shares`}
            aria-pressed={parseInt(quantity) === q}
      ...
      {/* Portfolio-percentage quick size */}
      <QuickSizeBar
        portfolioValue={portfolioValue}
        currentPrice={price}
        onSelectShares={(shares) => setQuantity(String(shares))}
        currentShares={qty}
        className="-mx-3"
      />
      ...
      {/* Position Sizer: show when we have a market buy with a stop price defined */}
      {orderType === "market" && side === "buy" && price > 0 && parseFloat(stopPrice) > 0 && parseFloat(stopPrice) < price && (
        <PositionSizer
          entry={price}
          stopLoss={parseFloat(stopPrice)}
          takeProfit={parseFloat(takeProfitPrice) > price ? parseFloat(takeProfitPrice) : undefined}
        />
      )}

      {/* Risk/Reward Chart: show when stop and take-profit are both set */}
      {price > 0 && parseFloat(stopPrice) > 0 && parseFloat(stopPrice) < price && parseFloat(takeProfitPrice) > price && (
        <RiskRewardChart
          entry={price}
          stopLoss={parseFloat(stopPrice)}
      ...
      {/* Counterparty match card — shown for market orders */}
      {orderType === "market" && (
        <CounterpartyMatchCard
          side={side}
          orderReady={canExecuteMarket}
        />
      )}
