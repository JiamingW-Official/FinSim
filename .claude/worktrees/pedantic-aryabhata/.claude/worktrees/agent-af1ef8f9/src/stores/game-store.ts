...
interface GameState {
  ...
  updateStats: (patch: Partial<PlayerStats>) => Achievement[];
...
/** Helper: build Achievement objects from defs, awarding XP from def.xp */
function buildAchievements(
  defs: typeof ACHIEVEMENT_DEFS,
...
export const useGameStore = create<GameState>()(
  persist(
    (set, get) => ({
      ...
      recordTrade: (pnL, ticker, isShort, isLimitOrder, entryValue) => {
        const state = get();
        ...
        // 5-bagger tracking: compute trade multiple if entryValue provided
        const tradeMultiple =
          entryValue && entryValue > 0 ? (entryValue + pnL) / entryValue : 0;
        const newMaxTradeMultiple = Math.max(
          state.stats.maxTradeMultiple ?? 0,
          tradeMultiple,
        );

        // Intraday session trade counter (reset when date changes)
        const prevDate = state.stats.lastTradeDate;
        const intradayTradesSession =
          prevDate === today
            ? (state.stats.intradayTradesSession ?? 0) + 1
            : 1;
        ...
        const newStats: PlayerStats = {
          ...
          maxTradeMultiple: newMaxTradeMultiple,
          intradayTradesSession,
        };
        ...
        // Check achievements and add their XP
        const existingIds = new Set(state.achievements.map((a) => a.id));
        const newAchievements = buildAchievements(ACHIEVEMENT_DEFS, newStats, existingIds);
        const achievementXP = newAchievements.reduce((sum, a) => sum + (a.xp ?? 0), 0);
      ...
      recordAnalyticsView: (type) => {
        ...
        const existingIds = new Set(state.achievements.map((a) => a.id));
        const newAchievements = buildAchievements(ACHIEVEMENT_DEFS, newStats, existingIds);
        const achievementXP = newAchievements.reduce((sum, a) => sum + (a.xp ?? 25), 0);

        const newXP = state.xp + achievementXP;
        const newLevel = getLevelForXP(newXP);
      ...
      /**
       * Apply a partial stats patch and check for newly unlocked achievements.
       * Use this from external stores (learn-store, prediction-store, etc.) to
       * update stats fields like lessonsCompleted, flashcardsReviewed, etc.
       */
      updateStats: (patch) => {
        const state = get();
        const newStats: PlayerStats = { ...state.stats, ...patch };
        const existingIds = new Set(state.achievements.map((a) => a.id));
        const newAchievements = buildAchievements(ACHIEVEMENT_DEFS, newStats, existingIds);
        const achievementXP = newAchievements.reduce((sum, a) => sum + (a.xp ?? 0), 0);
      ...
      /**
       * Re-evaluate all achievement conditions against current stats and unlock
       * any that pass but haven't been awarded yet. Useful for bootstrapping
       * after a stats migration or when achievements are added post-release.
       */
      checkAndUnlockAchievements: () => {
        const state = get();
        const existingIds = new Set(state.achievements.map((a) => a.id));
        const newAchievements = buildAchievements(ACHIEVEMENT_DEFS, state.stats, existingIds);
