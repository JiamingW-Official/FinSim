...
import { PatternRadar, type PatternHistoryEntry } from "./PatternRadar";
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isActive
                  ? isBullSeg
                    ? score >= 40 ? "bg-green-500" : "bg-green-400"
        ...
        {top.map((s) => (
          <button
            ...
            className={cn(
              ...
              s.direction === "bullish"
                ? selectedId === s.id
                  ? "bg-green-500/30 text-green-300 border-green-400/60 ring-1 ring-green-400/40"
...
function LevelLadder({
  ...
  return (
    <div className="rounded-md bg-background/40 border border-border/40 px-2 py-1.5 space-y-0.5">
      ...
      {nearestSup && (
        <div className="flex items-center justify-between text-[9px]">
          <span className="text-green-400 font-bold">${nearestSup.price.toFixed(2)}</span>
          <span className="text-muted-foreground/70">{nearestSup.label}</span>
          <span className="text-green-400/60 text-[8px]">▼ S</span>
        </div>
      )}
    </div>
  );
...
function HabitsSection({ habits }: { habits: HabitObservation[] }) {
  ...
  return (
    <div className="rounded-md border border-border/40 bg-muted/20 px-2 py-1.5 space-y-1">
      ...
      {habits.map((h, i) => (
        <div key={i} className="flex gap-1.5 text-[9px] leading-tight">
          <span
            className={cn(
              "shrink-0 mt-0.5 font-black",
              h.positive ? "text-green-400" : "text-amber-400",
            )}
          >
            {h.positive ? "+" : "!"}
          </span>
          <span className="text-muted-foreground">{h.observation}</span>
        </div>
      ))}
    </div>
  );
...
const REGIME_CLASSES: Record<string, { bg: string; text: string; border: string }> = {
  strong_bull: { bg: "bg-green-500/15", text: "text-green-400", border: "border-green-500/30" },
...
function TradePlanCard({ plan }: { plan: TradePlan }) {
  return (
    ...
    >
      ...
      <div className="flex justify-between text-[9px]">
        <span className="text-green-400/80">Target 1</span>
        <span className="font-mono text-green-400">${plan.target1.toFixed(2)}</span>
      </div>
      <div className="flex justify-between text-[9px]">
        <span className="text-green-400/50">Target 2</span>
        <span className="font-mono text-green-400/60">${plan.target2.toFixed(2)}</span>
      </div>
      <div className="flex justify-between text-[9px]">
...
export function AICoachPanel() {
  // Auto-expand for advanced users who completed onboarding
  const experienceLevel = useOnboardingStore((s) => s.experienceLevel);
  const onboardingCompleted = useOnboardingStore((s) => s.completed);
  const [expanded, setExpanded] = useState(
    () => onboardingCompleted && experienceLevel === "advanced"
  );
  ...
  const [showPatternRadar, setShowPatternRadar] = useState(false);
  ...
  // When a new analysis result has patterns, prepend them to history (cap at 10).
  // Keyed by revealedCount so the same bar's patterns are only added once.
  const lastPatternBarRef = useRef(-1);
  useEffect(() => {
    if (!result || result.patterns.length === 0) return;
    if (lastPatternBarRef.current === revealedCount) return;
    lastPatternBarRef.current = revealedCount;

    const newEntries: PatternHistoryEntry[] = result.patterns.map((p) => ({
      pattern: p,
            ...
            <div className="flex flex-col gap-2 px-3 pb-3">
              ...
              {result && (
                <div className="space-y-2">
                  ...
                  {mode === "review" && result.grade ? (
                    ...
                    <>
                      ...
                      {result.patterns.length > 0 && (
                        <PatternChips patterns={result.patterns} />
                      )}

                      {/* Pattern Radar toggle */}
                      {(result.patterns.length > 0 || patternHistory.length > 0) && (
                        <button
                          type="button"
                          onClick={() => setShowPatternRadar((v) => !v)}
                          className="flex items-center gap-1 text-[8.5px] text-muted-foreground/60 hover:text-muted-foreground transition-colors"
                        >
                          <span>{showPatternRadar ? "▲" : "▼"}</span>
                          <span>
                            {showPatternRadar ? "Hide" : "Show"} Pattern Radar
                          </span>
                          {patternHistory.length > 0 && !showPatternRadar && (
                            <span className="rounded bg-muted px-1 py-px text-[7px] font-bold">
                      ...
                      {result.insights.length > 0 && !loading && (
                        <div className="space-y-1">
                          {result.insights.map((insight, i) => (
                            <motion.div
                              key={`insight-${i}-${insight.slice(0, 20)}`}
                  ...
                  {/* Behavioral habits */}
                  {result.habits && result.habits.length > 0 && !loading && (
                    <HabitsSection habits={result.habits} />
                  )}
                </div>
              )}
