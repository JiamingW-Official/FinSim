...
interface LeaderboardTableProps {
  ...
  onSelectOpponent?: (entry: RankedEntry) => void;
  selectedOpponentId?: string | null;
}

const INITIAL_SHOW = 20;

export function LeaderboardTable({ ranked, dimension, onSelectOpponent, selectedOpponentId }: LeaderboardTableProps) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card/50 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-3 py-2 border-b border-border/50">
        <span className="w-7 text-center text-[9px] font-bold uppercase tracking-widest text-muted-foreground">#</span>
        <span className="w-8" />
        <span className="flex-1 text-[9px] font-bold uppercase tracking-widest text-muted-foreground">Player</span>
        <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground">{dimConfig.shortLabel}</span>
        {onSelectOpponent && (
          <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/50 ml-1">
            Click to compare
          </span>
        )}
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {/* Use virtual scrolling when showing all rows (potentially 50+) */}
          {showAll && topRows.length > INITIAL_SHOW ? (
            <VirtualList
              items={topRows}
              itemHeight={48}
              overscan={8}
              ...
              renderItem={(entry, i) => (
                <RankRow
                  ...
                  isSelected={selectedOpponentId === entry.id}
                />
              )}
            />
          ) : (
