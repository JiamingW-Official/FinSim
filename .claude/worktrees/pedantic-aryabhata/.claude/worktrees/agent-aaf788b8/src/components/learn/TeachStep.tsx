  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        {blocks.map((block, bi) => {
          ...
          if (block.kind === "bullets" && block.items) {
            return (
              <motion.div
                key={bi}
                className="rounded-lg border border-border/40 bg-muted/25 px-4 py-3.5"
      ...
      <motion.button
        type="button"
        onClick={onContinue}
        className="mt-1 w-full rounded-lg bg-primary py-3.5 text-sm font-bold text-primary-foreground transition-all hover:brightness-110 hover:shadow-[0_0_20px_rgba(16,185,129,0.3)]"
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
