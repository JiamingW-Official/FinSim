...
export function ArenaPlayer({ config, opponent, onFinish, onCancel }: ArenaPlayerProps) {
  ...
  return (
    <div className={cn(
      ...
      {/* ═══ Top HUD ═══ */}
      <div className="flex flex-col border-b border-border/50 bg-background/80">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5">
          ...
          <div className="text-right">
            <motion.div
              ...
              initial={{ scale: 1.15 }}
              animate={{ scale: 1 }}
              className={cn("text-sm font-bold tabular-nums", totalPnl >= 0 ? "text-green-400" : "text-red-400")}
            >
              {totalPnl >= 0 ? "+" : ""}${totalPnl.toFixed(2)}
            </motion.div>
            <div className="text-[10px] text-muted-foreground/60 tabular-nums">{pnlPct >= 0 ? "+" : ""}{pnlPct.toFixed(2)}%</div>
          </div>

          {/* Position badge */}
          <div className="flex items-center gap-1 rounded-lg border border-border/50 bg-muted/10 px-2 py-1">
            {sim.position ? (
              <>
                {sim.position.side === "long"
                  ? <ArrowUpRight className="h-3 w-3 text-green-400" />
                  : <ArrowDownRight className="h-3 w-3 text-red-400" />}
                <span className={cn("text-[10px] font-bold", sim.position.side === "long" ? "text-green-400" : "text-red-400")}>
                  {sim.position.side === "long" ? "LONG" : "SHORT"} {sim.position.quantity}
                </span>
              </>
            ) : (
              <><Minus className="h-3 w-3 text-muted-foreground/60" /><span className="text-[10px] text-muted-foreground/60">FLAT</span></>
            )}
          </div>

          {/* Trades count */}
          <div className="rounded-lg border border-border/50 bg-muted/10 px-2 py-1 text-center">
            <div className="text-xs font-bold tabular-nums text-foreground/80">{sim.trades.length}</div>
            <div className="text-[9px] text-muted-foreground/60">trades</div>
          </div>

          {/* Finish */}
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            onClick={handleFinish} disabled={isFinished}
            className="flex items-center gap-1 rounded-lg bg-muted/30 px-3 py-1.5 text-xs font-bold text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground disabled:opacity-50">
        ...
        <div className="flex items-center gap-2 px-3 py-1 border-t border-border/20">
          {/* Player label */}
          <span className="text-[9px] font-bold text-muted-foreground w-8">YOU</span>

          {/* P&L comparison bar */}
          <div className="flex-1 flex items-center h-4 rounded-full bg-muted/15 overflow-hidden relative">
            {/* Center line */}
            <div className="absolute left-1/2 top-0 bottom-0 w-px bg-muted/50 z-10" />
            ...
            <div className="w-1/2 flex justify-end">
              <motion.div
                className={cn(
                  ...
                  totalPnl >= 0 ? "bg-green-500/40" : "bg-red-500/40",
                )}
            ...
            <div className="w-1/2 flex justify-start">
              <motion.div
                className={cn(
                  ...
                  oppState.pnl >= 0 ? "bg-green-500/25" : "bg-red-500/25",
                )}
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className={cn(
                ...
                playerAhead ? "text-green-400" : pnlDiff === 0 ? "text-muted-foreground" : "text-red-400",
              )}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[9px] font-bold text-muted-foreground">{opponent.name}</span>
          </div>

          {/* Opponent P&L */}
          <div className={cn("text-[10px] font-bold tabular-nums min-w-[50px] text-right", oppState.pnl >= 0 ? "text-green-400/60" : "text-red-400/60")}>
            {oppState.pnl >= 0 ? "+" : ""}${oppState.pnl.toFixed(0)}
          </div>

          {/* Opponent position */}
          <div className="flex items-center gap-0.5">
            {oppState.positionDir === "long" && <ArrowUpRight className="h-2.5 w-2.5 text-green-400/40" />}
            {oppState.positionDir === "short" && <ArrowDownRight className="h-2.5 w-2.5 text-red-400/40" />}
            {oppState.positionDir === "flat" && <Minus className="h-2.5 w-2.5 text-muted-foreground/40" />}
            <span className="text-[8px] text-muted-foreground/60 tabular-nums">{oppState.tradeCount}t</span>
          </div>
          ...
          <AnimatePresence mode="wait">
            <motion.span
              ...
              className="text-[8px] text-muted-foreground/60 italic min-w-[60px]"
            >
              {oppState.lastAction}
            </motion.span>
          </AnimatePresence>
          ...
          {Math.abs(pnlDiff) > 20 && (
            <motion.div
              ...
              className={cn(
                ...
                playerAhead ? "bg-green-500/10" : "bg-red-500/10",
              )}
            >
              <Flame className={cn("h-2.5 w-2.5", playerAhead ? "text-green-400" : "text-red-400")} />
              <span className={cn("text-[8px] font-bold", playerAhead ? "text-green-400" : "text-red-400")}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-full md:w-72 border-l border-border/50 overflow-y-auto">
          <MiniTradePanel
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <div className="mt-4 flex items-center gap-3">
                <button type="button" onClick={() => setShowQuitConfirm(false)}
                  className="rounded-lg border border-border bg-muted/30 px-4 py-2 text-xs font-bold text-foreground/80 transition-colors hover:bg-muted/50">
                  Continue
                </button>
