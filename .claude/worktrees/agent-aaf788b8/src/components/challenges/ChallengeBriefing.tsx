...
export function ChallengeBriefing({
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-8 px-4 max-w-md mx-auto">
      {/* Mission icon */}
      <motion.div
        className="flex h-16 w-16 items-center justify-center rounded-lg bg-primary/10 border border-primary/20"
      ...
      <motion.div
        className="w-full rounded-lg border border-border/50 bg-card/50 p-4"
      ...
      <motion.div
        className="w-full rounded-lg border border-border/50 bg-card p-4"
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.97 }}
        className="relative w-full max-w-xs rounded-lg bg-primary py-3.5 text-sm font-black uppercase tracking-wider text-primary-foreground transition-all hover:brightness-110 shadow-[0_4px_20px_rgba(16,185,129,0.3)]"
      >
        {/* Pulse ring */}
        <motion.span
