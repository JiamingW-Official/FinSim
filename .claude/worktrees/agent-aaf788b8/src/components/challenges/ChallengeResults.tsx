...
const GRADE_CONFIG = {
  ...
  A: { color: "text-green-400", bg: "bg-green-400/10", border: "border-green-400/30", label: "Excellent!", glow: "shadow-[0_0_30px_rgba(52,211,153,0.2)]", emoji: "⭐" },
...
export function ChallengeResults({
  ...
  return (
    ...
    >
      ...
      <motion.div
        className={cn(
          "glass mx-4 flex w-full max-w-sm flex-col items-center gap-5 rounded-lg border p-8",
          config.border,
          config.glow,
        )}
      ...
      >
        ...
        <motion.div
          className={cn(
            "flex h-24 w-24 items-center justify-center rounded-lg border-2 text-5xl font-black",
            config.border,
            config.bg,
            config.color,
        ...
        {/* Stats — card style */}
        <motion.div
          className="w-full space-y-1 rounded-lg border border-border/50 bg-card/50 p-3"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          <ResultRow label="Objectives" icon="🎯" delay={0.65}>
            <span className={cn(
              "font-bold tabular-nums",
              objectivesCompleted === objectivesTotal ? "text-green-400" : "text-foreground",
            )}>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          className="w-full rounded-lg bg-primary py-3.5 text-sm font-black uppercase tracking-wider text-primary-foreground transition-all hover:brightness-110 shadow-[0_4px_16px_rgba(16,185,129,0.25)]"
        >
          Continue
        </motion.button>
      </motion.div>
