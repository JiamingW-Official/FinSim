...
export function MiniTradePanel({
  ...
  return (
    <div className="glass-subtle rounded-lg border border-border/50 p-3 space-y-2.5">
      {/* Price + Cash */}
        ...
        {position && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <div className={cn(
              ...
              unrealizedPnL >= 0 ? "bg-green-500/5 border-green-500/20" : "bg-red-500/5 border-red-500/20",
            )}>
              <div className="flex items-center gap-1.5">
                ...
                <span className={cn("text-[10px] font-bold uppercase", position.side === "long" ? "text-green-400" : "text-red-400")}>
                  {position.side}
                </span>
              ...
              <motion.span key={Math.round(unrealizedPnL * 100)} initial={{ scale: 1.3 }} animate={{ scale: 1 }}
                className={cn("font-bold tabular-nums", unrealizedPnL >= 0 ? "text-green-400" : "text-red-400")}>
      ...
      {hasShortSupport && (
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-1">
            <span className="text-[10px] text-muted-foreground mr-0.5">Qty</span>
            <div className="flex items-center gap-0.5 rounded-lg border border-border bg-card">
              {QTY_OPTIONS.map((q) => (
                <button key={q} type="button" onClick={() => setQty(q)}
                  className={cn("px-2 py-1 text-[10px] font-bold transition-colors rounded-md",
          ...
          <div className="flex items-center gap-1.5">
            <motion.button type="button" onClick={handleBuy}
              ...
              whileTap={{ scale: 0.95 }}
              className="flex-1 flex items-center justify-center gap-1 rounded-lg bg-green-500 py-2 text-xs font-black text-white uppercase tracking-wide transition-all hover:brightness-110 disabled:opacity-30 shadow-[0_2px_8px_rgba(16,185,129,0.25)]">
              <ArrowUpRight className="h-3 w-3" />
              Long {qty}
              {enableKeyboard && <kbd className="ml-1 rounded bg-muted/70 px-1 py-0.5 text-[8px] font-mono leading-none">W</kbd>}
            </motion.button>
            <motion.button type="button" onClick={handleShort}
              disabled={isLong || cash < currentPrice * qty || allComplete || atEnd}
              whileTap={{ scale: 0.95 }}
              className="flex-1 flex items-center justify-center gap-1 rounded-lg bg-red-500 py-2 text-xs font-black text-white uppercase tracking-wide transition-all hover:brightness-110 disabled:opacity-30 shadow-[0_2px_8px_rgba(239,68,68,0.25)]">
              <ArrowDownRight className="h-3 w-3" />
              Short {qty}
              {enableKeyboard && <kbd className="ml-1 rounded bg-muted/70 px-1 py-0.5 text-[8px] font-mono leading-none">S</kbd>}
            </motion.button>
          </div>
            ...
            {position && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
                <motion.button type="button" onClick={handleClose}
                  ...
                  className="w-full flex items-center justify-center gap-1.5 rounded-lg border border-border bg-muted/30 py-1.5 text-[11px] font-bold text-foreground/80 uppercase tracking-wide transition-all hover:bg-muted/50 disabled:opacity-30">
                  <X className="h-3 w-3" />
                  Close {position.side === "long" ? "Long" : "Short"} ({position.quantity})
                  {enableKeyboard && <kbd className="ml-1 rounded bg-muted/60 px-1 py-0.5 text-[8px] font-mono leading-none text-muted-foreground">Q</kbd>}
                </motion.button>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="flex items-center gap-2">
          {showLegacyBuy && (
            <>
              <div className="flex items-center gap-0.5 rounded-lg border border-border bg-card">
                {[1, 5, 10].map((q) => (
                  <button key={q} type="button" onClick={() => setQty(q)}
                    className={cn("px-2.5 py-1.5 text-[10px] font-bold transition-colors rounded-md",
                      qty === q ? "bg-primary/15 text-primary" : "text-muted-foreground hover:text-foreground")}>
                    {q}
                  </button>
                ))}
              ...
              <motion.button type="button" onClick={handleBuy}
                ...
                whileTap={{ scale: 0.95 }}
                className="flex-1 rounded-lg bg-green-500 py-2 text-xs font-black text-white uppercase tracking-wide transition-all hover:brightness-110 disabled:opacity-40 shadow-[0_2px_8px_rgba(16,185,129,0.25)]">
                Buy {qty}
              </motion.button>
            </>
          )}
          {showLegacySell && position && (
            <motion.button type="button" onClick={handleSell}
              disabled={allComplete || atEnd} whileTap={{ scale: 0.95 }}
              className="flex-1 rounded-lg bg-red-500 py-2 text-xs font-black text-white uppercase tracking-wide transition-all hover:brightness-110 disabled:opacity-40 shadow-[0_2px_8px_rgba(239,68,68,0.25)]">
        ...
        <div className="flex flex-wrap items-center gap-1">
          {indicatorChips.map((ind) => (
            <button key={ind.id} type="button" onClick={() => onToggleIndicator(ind.id)}
              className={cn(
                "rounded-full px-2 py-0.5 text-[9px] font-bold transition-all border",
          ...
          <div className="h-2 w-full rounded-full bg-muted/50 overflow-hidden">
            <motion.div
              className={cn("h-full rounded-full transition-colors",
                isPlaying ? "bg-gradient-to-r from-amber-400 via-amber-500 to-amber-400"
                  : atEnd ? "bg-green-500" : "bg-primary/60")}
          ...
          <div className="flex items-center gap-2">
            <motion.button type="button" onClick={isPlaying ? onPause : onPlay} disabled={atEnd}
              ...
              className={cn("relative flex h-10 w-10 items-center justify-center rounded-full font-bold transition-all disabled:opacity-30 shrink-0",
                isPlaying ? "bg-amber-500 text-white shadow-[0_0_16px_rgba(245,158,11,0.5)]"
            ...
            <motion.div layout className={cn(
              ...
              isPlaying ? "bg-amber-500/10 text-amber-400"
                : atEnd ? "bg-green-500/10 text-green-400" : "bg-muted/40 text-muted-foreground")}>
              {isPlaying && (
