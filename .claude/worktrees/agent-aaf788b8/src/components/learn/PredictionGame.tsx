...
export function PredictionGame({ onClose }: PredictionGameProps) {
  ...
  if (phase === "summary") {
    ...
    return (
      ...
      >
        <motion.div
          className="flex h-16 w-16 items-center justify-center rounded-lg bg-green-500/15 border border-green-500/30"
        ...
        <div className="grid grid-cols-3 gap-3 w-full max-w-xs">
          <SummaryStatBox label="Correct" value={`${sessionCorrect}/${ROUNDS}`} color="text-green-400" />
          <SummaryStatBox label="Accuracy" value={`${accuracy}%`} color={accuracy >= 70 ? "text-green-400" : "text-amber-400"} />
          <SummaryStatBox label="XP Earned" value={`+${sessionXP}`} color="text-purple-400" />
        </div>
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      ...
      {/* Chart */}
      <div className="relative h-48 rounded-lg border border-border bg-card/50 overflow-hidden">
          ...
          {visibleBars.map((bar, i) => {
            ...
            return (
              <div key={i} className="flex-1 relative h-full">
                {/* Wick */}
                <div
                  className={cn("absolute left-1/2 -translate-x-1/2 w-px", isUp ? "bg-green-400/60" : "bg-rose-400/60")}
        ...
        <AnimatePresence>
          {result && (
            ...
            >
              <motion.div
                className={cn(
                  "rounded-lg px-4 py-2 font-black text-sm",
                  result === "correct"
        ...
        {phase === "predict" && (
          ...
          >
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 rounded-lg border border-green-500/30 bg-green-500/10 py-3.5 text-sm font-black text-green-400 transition-all hover:bg-green-500/20 active:scale-95"
            >
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 rounded-lg border border-rose-500/30 bg-rose-500/10 py-3.5 text-sm font-black text-rose-400 transition-all hover:bg-rose-500/20 active:scale-95"
            >
