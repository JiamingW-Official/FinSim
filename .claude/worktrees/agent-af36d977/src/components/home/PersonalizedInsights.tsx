      ...
      {(profile.strengths.length > 0 || profile.weaknesses.length > 0) && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {profile.strengths.slice(0, 2).map((s) => (
            <div key={s} className="flex items-start gap-1.5 rounded-lg border border-green-500/10 bg-green-500/5 px-2.5 py-2">
              <CheckCircle2 className="h-3 w-3 text-green-400 mt-0.5 shrink-0" />
              <span className="text-[10px] text-green-400/80">{s}</span>
            </div>
          ))}
          ...
          {profile.nextActions.map((action) => {
            ...
            return (
              <Link key={action.title} href={action.href}>
                <div className="flex items-center gap-3 rounded-lg border border-border bg-card p-2.5 hover:bg-accent/30 transition-colors cursor-pointer">
