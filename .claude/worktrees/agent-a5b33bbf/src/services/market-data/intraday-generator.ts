 ...
 *
 * Realistic microstructure features:
 * - Opening gap from previous day's close (small random gap)
 * - Wider spreads/wicks at open, tighter midday
 * - U-shaped volume (high at open, low midday, high at close)
 * - Price-move correlated volume, spike clustering
 * - Strict OHLC validity: L <= min(O,C) and max(O,C) <= H
 */
export function generateIntradayBars(
  ...
  for (let dayIdx = 0; dayIdx < dailyBars.length; dayIdx++) {
    ...
    // Real markets gap from previous close; simulate a small random gap
    const prevClose = dayIdx > 0 ? dailyBars[dayIdx - 1].close : open;
    const gapPct = (rand() - 0.5) * 0.006; // up to ±0.3% gap
    const gapOpen = dayIdx > 0
      ? prevClose * (1 + gapPct)
      : open;
    // Clamp gap-adjusted open within the day's range
    const effectiveOpen = Math.max(low, Math.min(high, gapOpen));
    ...
    // Wider noise early (opening volatility) and late (closing auction), tighter midday
    const sigma = range * 0.14; // base wiggle amplitude
    for (let i = 1; i < n; i++) {
      const t = i / n;
      const drift = effectiveOpen + (close - effectiveOpen) * t;
      // Microstructure: opening 15% and closing 15% of session have 1.6x noise
      const sessionPos = i / n;
      const microNoiseMult = (sessionPos < 0.15 || sessionPos > 0.85) ? 1.6 : 1.0;
      const noise = (rand() - 0.5) * 2 * sigma * microNoiseMult * Math.sqrt(t * (1 - t) * 4);
      pts[i] = drift + noise;
    }
    ...
    if (ptRange > 0) {
      ...
      // Flat day
      for (let i = 0; i <= n; i++) pts[i] = effectiveOpen + (close - effectiveOpen) * (i / n);
    }
    ...
    // ── Build OHLCV bars with realistic wicks + spread microstructure ──────
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Spread microstructure: wider wicks at open and close, tighter midday
      const sessionFrac = i / Math.max(n - 1, 1);
      const spreadMult = (sessionFrac < 0.12 || sessionFrac > 0.88) ? 1.5
        : (sessionFrac < 0.25 || sessionFrac > 0.75) ? 1.15
        : 0.8;
      ...
      // Base wick size scales with both body and daily range, modulated by spread
      const wickBase = Math.max(body * 0.3, range * 0.012) * spreadMult;
      ...
      // Compute high/low, clamped to daily range
      let bHigh = Math.min(bMax + upperWickRaw, high);
      let bLow = Math.max(bMin - lowerWickRaw, low);
