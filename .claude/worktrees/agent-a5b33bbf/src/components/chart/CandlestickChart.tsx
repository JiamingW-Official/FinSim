...
export function CandlestickChart() {
  ...
  // Primary price series — type changes with chartStyle
  const priceSeriesRef = useRef<PriceSeries | null>(null);
  // Keep a typed ref for candlestick so markers plugin can attach (markers need a series ref)
  ...
  const prevChartStyleRef = useRef<ChartStyle>("candles");
  ...
  /** Build a price series of the requested style on the given chart instance. */
  const createPriceSeries = useCallback(
    (chart: IChartApi, style: ChartStyle): PriceSeries => {
      switch (style) {
        case "bars":
  ...
  useEffect(() => {
    ...
    // Use chartStyle from the store (read via ref to avoid re-running effect)
    const initialStyle = useChartStore.getState().chartStyle;
    const priceSeries = createPriceSeries(chart, initialStyle);
    prevChartStyleRef.current = initialStyle;
    priceSeriesRef.current = priceSeries;
    if (initialStyle === "candles") {
      candleSeriesRef.current = priceSeries as ISeriesApi<"Candlestick">;
    }
  ...
  // Swap the price series when chartStyle changes (after initial mount)
  useEffect(() => {
    const chart = chartRef.current;
    if (!chart) return;
    if (prevChartStyleRef.current === chartStyle) return;

    // Detach markers if any
    ...
    if (priceLineRef.current && priceSeriesRef.current) {
      try { (priceSeriesRef.current as ISeriesApi<"Candlestick">).removePriceLine(priceLineRef.current); } catch { /* ignore */ }
    ...
    // Create new series and set current data
    const newSeries = createPriceSeries(chart, chartStyle);
    priceSeriesRef.current = newSeries;
    candleSeriesRef.current = chartStyle === "candles" ? (newSeries as ISeriesApi<"Candlestick">) : null;
    prevChartStyleRef.current = chartStyle;

    // Re-populate data from displayBars (current state)
    const bars = useMarketDataStore.getState().allData.slice(
      0,
      useMarketDataStore.getState().revealedCount,
    );
    if (bars.length > 0) {
      if (chartStyle === "candles" || chartStyle === "bars") {
        (newSeries as ISeriesApi<"Candlestick">).setData(
          bars.map((bar) => ({
        ...
        (newSeries as ISeriesApi<"Line">).setData(
          bars.map((bar) => ({
            time: (bar.timestamp / 1000) as UTCTimestamp,
            value: bar.close,
          })),
  ...
  // Update price/volume data when displayBars changes
  useEffect(() => {
    const priceSeries = priceSeriesRef.current;
    if (!priceSeries || !volumeSeriesRef.current) return;
  ...
  }, [displayBars, isPlaying, chartStyle]);
  ...
  useEffect(() => {
    ...
    if (markers.length > 0 && priceSeriesRef.current) {
      // createSeriesMarkers accepts any ISeriesApi
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      markersPluginRef.current = createSeriesMarkers(priceSeriesRef.current as any, markers);
    }
  ...
  useEffect(() => {
    ...
    if (priceLineRef.current) {
      try { (series as ISeriesApi<"Candlestick">).removePriceLine(priceLineRef.current); } catch { /* ignore */ }
      priceLineRef.current = null;
    }
    ...
    if (position) {
      priceLineRef.current = (series as ISeriesApi<"Candlestick">).createPriceLine({
  ...
  useEffect(() => {
    const series = priceSeriesRef.current;
    if (!series) return;
    // price lines are only available on Candlestick/Line/Area/Bar series
    const s = series as ISeriesApi<"Candlestick">;

    // Remove old pending lines
    for (const [, line] of pendingLinesRef.current) {
      try { s.removePriceLine(line); } catch { /* already removed */ }
    }
    ...
    for (const order of pendingOrders) {
      ...
      const line = s.createPriceLine({ price: triggerPrice, color, lineWidth: 1, lineStyle, axisLabelVisible: true, title });
